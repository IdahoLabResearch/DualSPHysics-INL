...
// //-----------------------------------
// // Hypo model parameter initialization
//   // HypoModel = false;
//   Hypo_angle = 0;
//   Hypo_hs =0;
//   Hypo_n=0;
...
//  //-----------------------------------
  ...
  SymplecticDtPre=0;
  DemDtForce=0;  //(DEM)
  Time_inc =0; //hypoplasticity & elasticity
  MaxNumbers.Clear();
...
void JSph::LoadConfigParameters(const JXml *xml){
  ...
  switch(eparms.GetValueInt("ViscoTreatment",true,1)){
    ...
    case 4:  TVisco=VISCO_Hypoplasticity;     break;  //<hypoplasticity>
    case 5:  TVisco=VISCO_Elasticity;  break; //<elasticity>
  ...
  if(TBoundary==BC_MDBC){
    ...
    switch(eparms.GetValueInt("SlipMode",true,1)){
      ...
      case 4: SlipMode=SLIP_Friction_Soleimani;   break;
      case 5: SlipMode=SLIP_Friction_Zhan;  break;
      case 6: SlipMode=SLIP_NoSlip_Zhan; break;
      case 7: SlipMode=SLIP_FreeSlip_Zhan; break;
...
//   //---------------------------------------------------------------- 
//   //-configuration of hypoplasticity model
//   //-the default values are for Karlsruhe sand
//   if(xml.GetNodeSimple("case.execution.special.hypoplasticity",true)){
//     JCaseEParms eparms;
//     eparms.LoadXml(&xml,"case.execution.special.hypoplasticity");
//     Hypo_angle=eparms.GetValueFloat("Hypo_angle",true,30);
//     Hypo_hs=eparms.GetValueFloat("Hypo_hs",true,5800000000);
//     Hypo_n=eparms.GetValueFloat("Hypo_n",true,0.28);
//     Hypo_ed0=eparms.GetValueFloat("Hypo_ed0",true,0.53);
//     Hypo_ec0=eparms.GetValueFloat("Hypo_ec0",true,0.84);
//     Hypo_ei0=eparms.GetValueFloat("Hypo_ei0",true,1);
//     Hypo_alpha=eparms.GetValueFloat("Hypo_alpha",true,0.13);
//     Hypo_beta=eparms.GetValueFloat("Hypo_beta",true,1.0);
//     Hypo_initStres.x = eparms.GetValueFloat("Hypo_initStres_x",true,1.0);
//     Hypo_initStres.y = eparms.GetValueFloat("Hypo_initStres_y",true,1.0);
//     Hypo_initStres.z = eparms.GetValueFloat("Hypo_initStres_z",true,1.0);
//     Hypo_rhoparticle = eparms.GetValueFloat("Hypo_rhoparticle",true,1.0);
//   }
//  //----------------------------------------------------------------
  ...
  if(MultiPhase) {
    ...
    if(xml.GetNodeSimple("case.execution.special.hypoplasticity",true)) {
      ...
      if(TVisco==VISCO_Artificial)Log->PrintWarning("You are running non-Newtonian formulations using artificial viscosity.");
      InitHypoPhase(&xml,"case.execution.special.nnphases");
      ConfigConstantsMP_Hypo();
    }
    if(xml.GetNodeSimple("case.execution.special.elasticity",true)) {
      //For v5.0 NN version disable some options
      if(InOut)Run_Exceptioon("Multiphase formulations are not supported with inlet/outlet option.");
      // if(TBoundary==BC_MDBC)Run_Exceptioon("Multiphase formulations are not supported with BC_mDBC.");x
      if(TVisco==VISCO_Artificial)Log->PrintWarning("You are running non-Newtonian formulations using artificial viscosity.");
      InitElasticPhase(&xml,"case.execution.special.nnphases");
      ConfigConstantsMP_Elastic();
    }
  }
...
void JSph::LoadBoundNormals(unsigned np,unsigned npb,const unsigned *idp
  ,const typecode *code,tfloat3 *boundnormal, bool *BoundCorner)
{
  memset(boundnormal,0,sizeof(tfloat3)*np);
  memset(BoundCorner,0,sizeof(bool)*np);
  ...
  if(fun::FileExists(filenordata)){
    ...
    if(pnorsize){
      ...
      for(unsigned p=0;p<npb;p++){
        boundnormal[p]=ToTFloat3(pnor[p]);  //-For fixed and moving particles.
        BoundCorner[p]=false; // Initially set all the boundary particles to non-corner.
        }
...
void JSph::ConfigBoundNormals(unsigned np,unsigned npb,const tdouble3 *pos
  ,const unsigned *idp,tfloat3 *boundnormal,bool *boundcorner) 
  {
...
void JSph::VisuConfig(){
 ...
 if(MultiPhase){ //<vs_non-Newtonian_ini>
  ...
  //   Log->Print(fun::VarStr("Internal friction angle",Hypo_angle));
  //   Log->Print(fun::VarStr("Grain hardness",Hypo_hs));
  //   Log->Print(fun::VarStr("Exponent n",Hypo_n));
  //   Log->Print(fun::VarStr("Minimum void ratio",Hypo_ed0));
  //   Log->Print(fun::VarStr("Critical void ratio",Hypo_ec0));
  //   Log->Print(fun::VarStr("Maximum void ratio",Hypo_ei0));
  //   Log->Print(fun::VarStr("Exponent Alpha",Hypo_alpha));
  //   Log->Print(fun::VarStr("Exponent Beta",Hypo_beta));
  //   Log->Print(fun::VarStr("Particle density",Hypo_rhoparticle));
  //   Log->Print(fun::VarStr("Initial stress",Hypo_initStres)); /// Need to check whether this vector can be printed or not
  // }
  //-DensityDiffusion.
...
void JSph::RunInitialize(unsigned np,unsigned npb,const tdouble3 *pos,const unsigned *idp
  ,const typecode *code,tfloat4 *velrhop,tfloat3 *boundnormal,bool *BoundCorner)
{
  if(!PartBegin){
    JXml xml; xml.LoadFile(FileXml);
    ...
    if(xml.GetNodeSimple("case.execution.special.initialize",true)){
      ...
      if(init.Count()){
        ...
        init.Run(np,npb,pos,idp,mktype,velrhop,boundnormal,BoundCorner);
...
void JSph::ConfigSaveData(unsigned piece,unsigned pieces,std::string div){
  ...
  switch(TVisco){
    ...
    case VISCO_Hypoplasticity: parthead.ConfigVisco(JPartDataHead::VISCO_LaminarSPS,Visco,ViscoBoundFactor);  break; //<vs_non-Newtonian>
    case VISCO_Elasticity:  parthead.ConfigVisco(JPartDataHead::VISCO_LaminarSPS,Visco,ViscoBoundFactor); break;
  ...
  ,unsigned ndom,const tdouble3 *vdom,const StInfoPartPlus *infoplus)
{
  ...
  if(DataBi4){
    ...
    if(SvData&SDAT_Binx){
      ...
      // if(!(err=arrays.CheckErrorArray("SigmaInv" ,TypeFloat3 ,npok)).empty())Run_Exceptioon(err);
      ...
      // const tfloat3  *sigInv =arrays.GetArrayFloat3 ("SigmaInv");
      if(SvPosDouble){
  ...
  ,const tdouble3 *pos,const unsigned *idp,const tfloat3 *boundnormal)const
{
  if(JVtkLib::Available()){
    ...
    //bool*     vnorc=arrays.CreateArrayPtrBool   ("NormalCorner",npsel);
    ...
    //memcpy(vnorc,boundcorner,sizeof(bool)*npb);
    //-Computes normalsize.
...
std::string JSph::GetSlipName(TpSlipMode tslip){
  ...
  else if(tslip==SLIP_Friction_Soleimani)tx="Frictional slip Soleimani method";
  else if(tslip==SLIP_Friction_Zhan)tx="Frictional slip Zhan method";
  else if(tslip==SLIP_NoSlip_Zhan)tx="No-slip Zhan method";
  else if(tslip==SLIP_FreeSlip_Zhan)tx="Free slip Zhan method";
  else tx="???";
  return(tx);
}
...
//==============================================================================
// Load related data and adjusts Multi-phase fluid density for hypoplasticity
//==============================================================================
void JSph::InitHypoPhase(const JXml *sxml,std::string xmlpath) {
  Log->Print("");
  Log->Print("[Hypo-phase configuration]\n");
  TiXmlNode* node=sxml->GetNodeSimple("case.execution.special.hypoplasticity");
  sxml->CheckElementNames(node->ToElement(),true,"*phase");
  PhaseCount=sxml->CountElements(node,"phase");

  Log->Printf("==: PhaseCount:%d",PhaseCount);
  PhaseHypo=new StPhaseHypo[PhaseCount]; 	memset(PhaseHypo,0,sizeof(StPhaseHypo)*PhaseCount);
  if(PhaseCount<1)Run_Exceptioon("The number of phases is invalid.");
  TiXmlElement* ele=node->FirstChildElement("phase");
  for(unsigned c=0; ele; c++) {
    sxml->CheckElementNames(ele,true,"rhop csound visco Hypo_angle Hypo_wallfriction Hypo_hs Hypo_n Hypo_ed0 Hypo_ec0 Hypo_ei0 Hypo_alpha Hypo_beta Hypo_voidRatio Hypo_rhoparticle Hypo_initStres_x Hypo_initStres_y Hypo_initStres_z phasetype");
    const word mkfluid=sxml->GetAttributeWord(ele,"mkfluid");
    PhaseHypo[c].mkfluid=mkfluid;
    unsigned cmk=MkInfo->GetMkBlockByMkFluid(PhaseHypo[c].mkfluid);
    if(cmk>=MkInfo->Size())Run_Exceptioon(fun::PrintStr("No particles with mkfluid=%u",mkfluid));
    const JSphMkBlock* bk=MkInfo->Mkblock(cmk);

    //PhaseArray holds physical proprerties of phases
    PhaseHypo[c].phaseid=c;
    PhaseHypo[c].rho=sxml->ReadElementFloat(ele,"rhop","value");
    //PhaseHypo[c].mass_ph to be setup in ConfigConstants
    // printf("mark - check mass calculation here");
    PhaseHypo[c].Cs0=sxml->ReadElementFloat(ele,"csound","value",true);
    //Parameter to tune for artificial viscosity
    PhaseHypo[c].visco=sxml->ReadElementFloat(ele,"visco","value");

    // PhaseCte holds const. eq. multiphase constants
    PhaseHypo[c].idbegin=bk->Begin;
    PhaseHypo[c].count=bk->Count;
    PhaseHypo[c].Hypo_wallfriction = sxml->ReadElementFloat(ele,"Hypo_wallfriction","value");
    PhaseHypo[c].Hypo_hs=sxml->ReadElementFloat(ele,"Hypo_hs","value");
    PhaseHypo[c].Hypo_angle=sxml->ReadElementFloat(ele,"Hypo_angle","value");
    PhaseHypo[c].Hypo_n= sxml->ReadElementFloat(ele, "Hypo_n", "value");
    PhaseHypo[c].Hypo_ed0=sxml->ReadElementFloat(ele,"Hypo_ed0","value");
    PhaseHypo[c].Hypo_ec0=sxml->ReadElementFloat(ele,"Hypo_ec0","value");
    PhaseHypo[c].Hypo_ei0=sxml->ReadElementFloat(ele,"Hypo_ei0","value");
    PhaseHypo[c].Hypo_alpha=sxml->ReadElementFloat(ele,"Hypo_alpha","value");
    PhaseHypo[c].Hypo_beta=sxml->ReadElementFloat(ele,"Hypo_beta","value");
    PhaseHypo[c].Hypo_rhoparticle=sxml->ReadElementFloat(ele,"Hypo_rhoparticle","value");
    PhaseHypo[c].Hypo_voidRatio=sxml->ReadElementFloat(ele,"Hypo_voidRatio","value");
    PhaseHypo[c].Hypo_initStres.x=sxml->ReadElementFloat(ele,"Hypo_initStres_x","value");
    PhaseHypo[c].Hypo_initStres.y=sxml->ReadElementFloat(ele,"Hypo_initStres_y","value");
    PhaseHypo[c].Hypo_initStres.z=sxml->ReadElementFloat(ele,"Hypo_initStres_z","value");
    PhaseHypo[c].phasetype=sxml->ReadElementUnsigned(ele,"phasetype","value");   //only 0 in v 5.0 supported
    ele=ele->NextSiblingElement("phase");
  }
  //-Sort phases according mkfluid.
  for(unsigned c=0; c<PhaseCount-1; c++)for(unsigned c2=c+1; c2<PhaseCount; c2++)if(PhaseHypo[c].mkfluid>PhaseHypo[c2].mkfluid){
    StPhaseHypo a=PhaseHypo[c];
    PhaseHypo[c]=PhaseHypo[c2];
    PhaseHypo[c2]=a;
  }
  //-Shows phase information.
  Log->Printf("PhaseCount:%d",PhaseCount);
  for(unsigned c=0; c<=PhaseCount-1; c++) {
    const StPhaseHypo &ar=PhaseHypo[c];
    Log->Printf("Phase %d",c);
    Log->Printf("  Rho......: %f",ar.rho);
    if(ar.Cs0)Log->Printf("  Cs0......: %f",ar.Cs0);
    // if(ar.Gamma)Log->Printf("  Gamma....: %f",ar.Gamma);
    Log->Printf("  artificial alpha: %f",ar.visco);
    Log->Printf("  friction angle: %f",ar.Hypo_angle);
    Log->Printf("  hypoplasticity hs..: %f",ar.Hypo_hs);
    Log->Printf("  hypoplasticity n..: %f",ar.Hypo_n);
    Log->Printf("  hypoplasticity ed_0..: %f",ar.Hypo_ed0);
    Log->Printf("  hypoplasticity ec_0..: %f",ar.Hypo_ec0);
    Log->Printf("  hypoplasticity ei_0..: %f",ar.Hypo_ei0);
    Log->Printf("  hypoplasticity alpha..: %f",ar.Hypo_alpha);
    Log->Printf("  hypoplasticity beta..: %f",ar.Hypo_beta);
    Log->Printf("  hypoplasticity rho_particle..: %f",ar.Hypo_rhoparticle);
    Log->Printf("  hypoplasticity intial void ratio..: %f",ar.Hypo_voidRatio);
    Log->Printf("  Initial stress..(%f,%f,%f)",ar.Hypo_initStres.x, ar.Hypo_initStres.y, ar.Hypo_initStres.z);
  }
  Log->Printf("Dt relaxation parameter: %f",lamda);
  Log->Print("");
}
...
//==============================================================================
// Load related data and adjusts Multi-phase fluid density for elasticity
//==============================================================================
void JSph::InitElasticPhase(const JXml *sxml,std::string xmlpath) {
  Log->Print("");
  Log->Print("[Elastic-phase configuration]\n");
  TiXmlNode* node=sxml->GetNodeSimple("case.execution.special.elasticity");
  sxml->CheckElementNames(node->ToElement(),true,"*phase");
  PhaseCount=sxml->CountElements(node,"phase");

  Log->Printf("==: PhaseCount:%d",PhaseCount);
  PhaseElastic=new StPhaseElastic[PhaseCount]; 	memset(PhaseElastic,0,sizeof(StPhaseElastic)*PhaseCount);
  if(PhaseCount<1)Run_Exceptioon("The number of phases is invalid.");
  TiXmlElement* ele=node->FirstChildElement("phase");
  for(unsigned c=0; ele; c++) {
    sxml->CheckElementNames(ele,true,"rhop csound visco lameparm1 lameparm2 Elastic_initStres_x Elastic_initStres_y Elastic_initStres_z Elastic_wallfriction phasetype");
    const word mkfluid=sxml->GetAttributeWord(ele,"mkfluid");
    PhaseElastic[c].mkfluid=mkfluid;
    unsigned cmk=MkInfo->GetMkBlockByMkFluid(PhaseElastic[c].mkfluid);
    if(cmk>=MkInfo->Size())Run_Exceptioon(fun::PrintStr("No particles with mkfluid=%u",mkfluid));
    const JSphMkBlock* bk=MkInfo->Mkblock(cmk);

    //PhaseArray holds physical proprerties of phases
    PhaseElastic[c].phaseid=c;
    PhaseElastic[c].rho=sxml->ReadElementFloat(ele,"rhop","value");
    //PhaseElastic[c].mass_ph to be setup in ConfigConstants
    // printf("mark - check mass calculation here");
    PhaseElastic[c].Cs0=sxml->ReadElementFloat(ele,"csound","value",true);
    //Parameter to tune for artificial viscosity
    PhaseElastic[c].visco=sxml->ReadElementFloat(ele,"visco","value");

    // PhaseCte holds const. eq. multiphase constants
    PhaseElastic[c].idbegin=bk->Begin;
    PhaseElastic[c].count=bk->Count;
    PhaseElastic[c].Elastic_wallfriction = sxml->ReadElementFloat(ele,"Elastic_wallfriction","value");
    PhaseElastic[c].lameparm1 = sxml->ReadElementFloat(ele,"lameparm1","value");
    PhaseElastic[c].lameparm2 = sxml->ReadElementFloat(ele,"lameparm2","value");
    PhaseElastic[c].Elastic_initStres.x=sxml->ReadElementFloat(ele,"Elastic_initStres_x","value");
    PhaseElastic[c].Elastic_initStres.y=sxml->ReadElementFloat(ele,"Elastic_initStres_y","value");
    PhaseElastic[c].Elastic_initStres.z=sxml->ReadElementFloat(ele,"Elastic_initStres_z","value");
    PhaseElastic[c].phasetype=sxml->ReadElementUnsigned(ele,"phasetype","value");   //only 0 in v 5.0 supported
    ele=ele->NextSiblingElement("phase");
  }
  //-Sort phases according mkfluid.
  for(unsigned c=0; c<PhaseCount-1; c++)for(unsigned c2=c+1; c2<PhaseCount; c2++)if(PhaseElastic[c].mkfluid>PhaseElastic[c2].mkfluid){
    StPhaseElastic a=PhaseElastic[c];
    PhaseElastic[c]=PhaseElastic[c2];
    PhaseElastic[c2]=a;
  }
  //-Shows phase information.
  Log->Printf("PhaseCount:%d",PhaseCount);
  for(unsigned c=0; c<=PhaseCount-1; c++) {
    const StPhaseElastic &ar=PhaseElastic[c];
    Log->Printf("Phase %d",c);
    Log->Printf("  Rho......: %f",ar.rho);
    if(ar.Cs0)Log->Printf("  Cs0......: %f",ar.Cs0);
    // if(ar.Gamma)Log->Printf("  Gamma....: %f",ar.Gamma);
    Log->Printf("  artificial alpha: %f",ar.visco);
    Log->Printf("  elasticity lame 1st parameter..: %f",ar.lameparm1);
    Log->Printf("  elasticity lame 2nd parameter..: %f",ar.lameparm2);
    Log->Printf("  Initial stress..(%f,%f,%f)",ar.Elastic_initStres.x, ar.Elastic_initStres.y, ar.Elastic_initStres.z);
    Log->Printf("  elasticity wall friction coefficient..: %f",ar.Elastic_wallfriction);
...
void JSph::ConfigConstantsMP_Hypo(){
  //Check if Cs0 is present for ALL phases
  bool Cs0_present=true;
  for(unsigned c=0; c<PhaseCount; c++)if(!PhaseHypo[c].Cs0)Cs0_present=false;
  if(Cs0_present){
    //Compute a new Cs0 for all system
    CSP.cs0=0;
    for(unsigned c=0; c<PhaseCount; c++) {
      CSP.cs0=max(CSP.cs0,double(PhaseHypo[c].Cs0));
      //CteB has already been calculated in InitMultiphase
      PhaseHypo[c].mass=float(PhaseHypo[c].rho*(Simulate2D ? Dp*Dp : Dp*Dp*Dp));
    }
  }
  CoefDtMin*=1.0e-5f;
  DtMin=(KernelH/Cs0)*CoefDtMin;
...
void JSph::ConfigConstantsMP_Elastic(){
  //Check if Cs0 is present for ALL phases
  bool Cs0_present=true;
  for(unsigned c=0; c<PhaseCount; c++)if(!PhaseElastic[c].Cs0)Cs0_present=false;
  if(Cs0_present){
    //Compute a new Cs0 for all system
    CSP.cs0=0;
    for(unsigned c=0; c<PhaseCount; c++) {
      CSP.cs0=max(CSP.cs0,double(PhaseElastic[c].Cs0));
      //CteB has already been calculated in InitMultiphase
      PhaseElastic[c].mass=float(PhaseElastic[c].rho*(Simulate2D ? Dp*Dp : Dp*Dp*Dp));
    }
  }
  CoefDtMin*=1.0e-5f;
  DtMin=(KernelH/Cs0)*CoefDtMin;
  ...
  ,tfloat4 *velrhop,float *auxNN)const
{
  ...
  if((TVisco!=VISCO_Hypoplasticity) & (TVisco!=VISCO_Elasticity))memset(auxNN,0,sizeof(float)*np);
  ...
  for(unsigned p=0; p<np; p++) {
    ...
    if(CODE_IsFluid(cod)) {
      ...
      if(TVisco== VISCO_Hypoplasticity){
        velrhop[p].w=PhaseHypo[cp].rho;
      }else if(TVisco== VISCO_Elasticity) velrhop[p].w=PhaseElastic[cp].rho;
      else{
