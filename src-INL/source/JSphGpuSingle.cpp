...
void JSphGpuSingle::ConfigDomain(){
  ...
  //<vs_non-Newtonian>
   if(MultiPhase)LoadMultiphaseData(Np, Idp, Code, Velrhop, AuxNN);

  //-Load normals for boundary particles (fixed and moving) in host memory
  tfloat3 *boundnormal=NULL;
  bool *BoundCorner = NULL;
  if(UseNormals){ 
    boundnormal=new tfloat3[Np];
    memset(boundnormal,0,sizeof(tfloat3)*Np);
    BoundCorner=new bool[Np];
    memset(BoundCorner,0,sizeof(bool)*Np);
    LoadBoundNormals(Np,Npb,Idp,Code,boundnormal,BoundCorner);
  }
  ...
  RunInitialize(Np,Npb,AuxPos,Idp,Code,Velrhop,boundnormal,BoundCorner);
  if(UseNormals)ConfigBoundNormals(Np,Npb,AuxPos,Idp,boundnormal,BoundCorner);
  ...
  ParticlesDataUp(Np,boundnormal,BoundCorner);
  //Log->Printf("Posxyg: %f %f", Posxyg[Npb].x,Posxyg[Npb].y);
  //Log->Printf("Velrhopg: %f %f %f %f", Velrhopg[Npb].x,Velrhopg[Npb].y,Velrhopg[Npb].z,Velrhopg[Npb].w);
  //Log->Printf("BoundNormalg: %f %f %f", BoundNormalg[Npb].x,BoundNormalg[Npb].y,BoundNormalg[Npb].z);
  //Log->Printf("Forceg: %f %f %f", Forceg[Npb].x,Forceg[Npb].y,Forceg[Npb].z);
  //Log->Printf("Void_ratiog: %f", Void_ratiog[Npb]);
  //delete[] boundnormal; boundnormal=NULL;
  //delete[] BoundCorner; BoundCorner=NULL;
...
void JSphGpuSingle::RunCellDivide(bool updateperiodic){
  ...
  if((!MultiPhase && TVisco==VISCO_LaminarSPS)||(TVisco == VISCO_Hypoplasticity) ||(TVisco == VISCO_Elasticity)) { //<vs_non-Newtonian>
    tsymatrix3f *spstaug=ArraysGpu->ReserveSymatrix3f();
    float3 *forceg=ArraysGpu->ReserveFloat3();
    CellDivSingle->SortDataArrays(SpsTaug,spstaug);
    CellDivSingle->SortDataArrays(Forceg,forceg);
    swap(SpsTaug,spstaug);  ArraysGpu->Free(spstaug);
    swap(Forceg,forceg);   ArraysGpu->Free(forceg);
  }
  if((MultiPhase) && (TVisco != VISCO_Hypoplasticity) && (TVisco != VISCO_Elasticity) ) {  //<vs_non-Newtonian>
    float *auxg=ArraysGpu->ReserveFloat();
    CellDivSingle->SortDataArrays(AuxNNg,auxg);
    swap(AuxNNg,auxg);  ArraysGpu->Free(auxg);
  }
  if(TVisco == VISCO_Hypoplasticity ) {  //<vs_non-Newtonian>
    float *auxvoid=ArraysGpu->ReserveFloat();
    CellDivSingle->SortDataArrays(Void_ratiog,auxvoid);
    swap(Void_ratiog,auxvoid);  ArraysGpu->Free(auxvoid);
  }
  if(UseNormals){
...
//==============================================================================
void JSphGpuSingle::Interaction_Forces(TpInterStep interstep, double time_inc, int Zhan_loop, bool regularize){
  //@Wencheng Jin, the program enters into MdbcBoundCorrection even the condition is not satisfied.
  // if (TBoundary == BC_MDBC && (MdbcCorrector || interstep != INTERSTEP_SymCorrector))
  //   MdbcBoundCorrection(); //-Boundary correction for mDBC.
  InterStep=interstep;
  PreInteraction_Forces(interstep,Zhan_loop,SpsTaug); /// jinw@prepare momeory that creates and destroys every iteration
  ...
  const StInterParmsg parms=StrInterParmsg(Simulate2D
    ...
    ,Visco_etag, SpsTaug, SpsGradvelg
    ,D_tensorg, AuxNNg
    ,W_tensorg, Void_ratiog
    ,Distance_ratiog
    ,ShiftPosfsg, VelMax, Pressg, Forceg, BoundNormalg, BoundCornerg,regularize
    ,NULL,NULL);

  if(!MultiPhase)cusph::Interaction_Forces(parms);
  else         cusphNN::Interaction_ForcesNN(parms,time_inc,Zhan_loop);  //<vs_non-Newtonian>
...
//==============================================================================
double JSphGpuSingle::ComputeStep_Ver(bool regularize){
  int Zhan_loop = 0;
  if(BoundCorr)BoundCorrectionData();      //-Apply BoundCorrection.
  double deltat=0;
  Interaction_Forces(INTERSTEP_Verlet,deltat,Zhan_loop,regularize);    //-Interaction.
...
//==============================================================================
double JSphGpuSingle::ComputeStep_Sym(bool regularize){
  ...
  if(BoundCorr)BoundCorrectionData();          //-Apply BoundCorrection.
  if (!Zhan_method) Interaction_Forces(INTERSTEP_SymPredictor,Time_inc,Zhan_loop,false);  //-Interaction.
  else{
    Zhan_loop = 1; // Indicator for Zhan_friction slip mode: 1: predictor step, ignore boundary
    //Log->Printf("Predictor - material-material");
    Interaction_Forces(INTERSTEP_SymPredictor,Time_inc,Zhan_loop,false);
  } 
  ...
  if (Zhan_method){
    ...
    PosInteraction_Forces(false); //-Free memory used for interaction.
    //Log->Printf("Predictor - material-bound");
    Interaction_Forces(INTERSTEP_SymPredictor,Time_inc,Zhan_loop,false);
  }
  ...
  DemDtForce=dt;                               //(DEM)
  RunCellDivide(true);
  if (!Zhan_method) Interaction_Forces(INTERSTEP_SymCorrector,Time_inc,Zhan_loop,false);  //-Interaction.
  else{
    Zhan_loop = 1; // Indicator for Zhan_friction slip mode: 1: predictor step, ignore boundary
    //Log->Printf("Corrector - material-material");
    Interaction_Forces(INTERSTEP_SymCorrector,Time_inc,Zhan_loop,regularize);
  }
  const double ddt_c=DtVariable(true);         //-Calculate dt of corrector step.
  if(Shifting)RunShifting(dt);                 //-Shifting.
  if(!Zhan_method) ComputeSymplecticCorr(dt);  //-Apply Symplectic-Corrector to particles (periodic particles become invalid).
  else{
    ...
    //Log->Printf("Corrector - material-bound");
    Interaction_Forces(INTERSTEP_SymCorrector,Time_inc,Zhan_loop,false);
...
void JSphGpuSingle::Run(std::string appname,JSphCfgRun *cfg,JLog2 *log){
  ...
  //--------------------------------------------------------------------------------
  LoadConfig(cfg); ///jinw@has loadCaceConfig function to load xml input parameters CPU+GPU shared code
  LoadCaseParticles();
  VisuConfig();
  ConfigDomain(); /// jinw@reserve GPU memory, memory upload to device with initial value assigned
  ConfigRunMode("Single-Gpu");
  VisuParticleSummary();

  //-Initialisation of execution variables. | Inicializacion de variables de ejecucion.
  //------------------------------------------------------------------------------------
  InitRunGpu(); /// jinw@assign initial state variables as stress and void ratio
  RunGaugeSystem(TimeStep,true);
  if(InOut)InOutInit(TimeStepIni);
  FreePartsInit();
  UpdateMaxValues();
  PrintAllocMemory(GetAllocMemoryCpu(),GetAllocMemoryGpu());
  SaveData(); // jinw@the extra variables interested
...
void JSphGpuSingle::SaveData(){
  ...
  if(save){
    ...
    /// need work on the stress and void ratio
  ...
  //// add stress components sigma/tau/viod ratio
  if (Sigma) arrays.AddArray("Sigma",npsave,Sigma);
  if (Void_ratio)  arrays.AddArray("Void",npsave,Void_ratio);
  if (Force) arrays.AddArray("Force", npsave, Force);
