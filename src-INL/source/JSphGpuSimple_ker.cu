...
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticPre
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,double TimeStep)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      if(!floating || CODE_IsFluid(rcode)){ //-Particles: Fluid.
        ...
        const float3 race=ace[p];
        if(TimeStep < 1){
          rvelrhopnew.x=float(double(rvelrhoppre.x) + (double(race.x)+gravity.x*TimeStep) * dtm);
          rvelrhopnew.y=float(double(rvelrhoppre.y) + (double(race.y)+gravity.y*TimeStep) * dtm);
          rvelrhopnew.z=float(double(rvelrhoppre.z) + (double(race.z)+gravity.z*TimeStep) * dtm);         
        }else{
...
void ComputeStepSymplecticPre(bool floating,bool shift,bool inout,unsigned np,unsigned npb
  ...
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,cudaStream_t stm,double TimeStep)
{
  if(np){
    dim3 sgrid=GetSimpleGridSize(np,SPHBSIZE);
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
        else        KerComputeStepSymplecticPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
      }
...
//------------------------------------------------------------------------------
/// Zhan's method, corr_zhanpre step
//------------------------------------------------------------------------------

template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticCorr_ZhanPre
  (unsigned n,unsigned npb
  ,float4 *velrhop,const float3 *ace,const float4 *shiftposfs
  ,const float3 *indirvel,double dtm,float3 gravity,typecode *code
  ,double2 *movxyg,double *movzg,double TimeStep)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    if(p>npb-1){//-Particles: Floating & Fluid.
      const typecode rcode=code[p];
      ...
      if(!floating || CODE_IsFluid(rcode)){ //-Particles: Fluid.
        ...
        const float3 race=ace[p];
        if(TimeStep < 1){
          rvelrhop.x=float(double(rvelrhop.x) + (double(race.x)+gravity.x*TimeStep) * dtm);
          rvelrhop.y=float(double(rvelrhop.y) + (double(race.y)+gravity.y*TimeStep) * dtm);
          rvelrhop.z=float(double(rvelrhop.z) + (double(race.z)+gravity.z*TimeStep) * dtm);         
        }else{
      ...
      //-Stores new velocity and density.
      velrhop[p].x=rvelrhop.x;
      velrhop[p].y=rvelrhop.y;
      velrhop[p].z=rvelrhop.z;
    }
  }
}

//==============================================================================
/// Updates particles using Zhan's method prediction step.
//==============================================================================   
void ComputeStepSymplecticCorr_ZhanPre(bool floating,bool shift,bool inout,unsigned np,unsigned npb
  ,float4 *velrhopg,const float3 *ace,const float4 *shiftposfs
  ,const float3 *indirvel,double dtm,tfloat3 gravity,typecode *code
  ,double2 *movxy,double *movz,cudaStream_t stm,double TimeStep)
{
  if(np){
    dim3 sgrid=GetSimpleGridSize(np,SPHBSIZE);
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticCorr_ZhanPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhopg,ace,shiftposfs,indirvel,dtm,Float3(gravity),code,movxy,movz,TimeStep);
        else        KerComputeStepSymplecticCorr_ZhanPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhopg,ace,shiftposfs,indirvel,dtm,Float3(gravity),code,movxy,movz,TimeStep);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticCorr_ZhanPre<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhopg,ace,shiftposfs,indirvel,dtm,Float3(gravity),code,movxy,movz,TimeStep);
        else        KerComputeStepSymplecticCorr_ZhanPre<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhopg,ace,shiftposfs,indirvel,dtm,Float3(gravity),code,movxy,movz,TimeStep);
      }
  ...
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving.
      ...
      if(!floating || CODE_IsFluid(rcode)){//-Particles: Fluid.
        ...
        const float3 race=ace[p];
        if(TimeStep < 1){
          rvelrhopnew.x=float(double(rvelrhoppre.x) + (double(race.x)+gravity.x*TimeStep) * dt);
          rvelrhopnew.y=float(double(rvelrhoppre.y) + (double(race.y)+gravity.y*TimeStep) * dt);
          rvelrhopnew.z=float(double(rvelrhoppre.z) + (double(race.z)+gravity.z*TimeStep) * dt);         
        }else{
...
//------------------------------------------------------------------------------
template<bool floating,bool shift,bool inout> __global__ void KerComputeStepSymplecticCor_ZhanCorr
  (unsigned n,unsigned npb
  ,const float4 *velrhoppre,const float *ar,const float4 *shiftposfs
  ,const float3 *indirvel,double dtm,double dt,float rhopzero,float rhopoutmin,float rhopoutmax
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(p<n){
    if(p<npb){ //-Particles: Fixed & Moving. Not necessary to update boundary density for Boundary particles
      double epsilon_rdot=(-double(ar[p])/double(velrhop[p].w))*dt;
      float rrhop=float(double(velrhoppre[p].w) * (2.-epsilon_rdot)/(2.+epsilon_rdot));
      rrhop=(rrhop<rhopzero? rhopzero: rrhop); //-To prevent absorption of fluid particles by boundaries. | Evita q las boundary absorvan a las fluidas.
      velrhop[p]=make_float4(0,0,0,rrhop);
      ...
      //-Updates density.
      const double epsilon_rdot=(-double(ar[p])/double(velrhop[p].w))*dt;
      const float rhopnew=float(double(velrhoppre[p].w) * (2.-epsilon_rdot)/(2.+epsilon_rdot));
      float4 rvelrhopnew;
      if(!floating || CODE_IsFluid(rcode)){//-Particles: Fluid.
        //-Calculate velocity. | Calcula velocidad.
        rvelrhopnew.x=float(double(velrhop[p].x));
        rvelrhopnew.y=float(double(velrhop[p].y));
        rvelrhopnew.z=float(double(velrhop[p].z));
        rvelrhopnew.w=rhopnew;
        //-Calculate displacement. | Calcula desplazamiento.
        double dx=(double(velrhoppre[p].x)+double(rvelrhopnew.x)) * dtm;
        double dy=(double(velrhoppre[p].y)+double(rvelrhopnew.y)) * dtm;
        double dz=(double(velrhoppre[p].z)+double(rvelrhopnew.z)) * dtm;
        if(shift){
          const float4 rshiftpos=shiftposfs[p];
          dx+=double(rshiftpos.x);
          dy+=double(rshiftpos.y);
  ...
  if(np){
    ...
    if(inout){
      ...
      if(shift){
        ...
        if(floating)KerComputeStepSymplecticCor<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
        else        KerComputeStepSymplecticCor<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,ace,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,Float3(gravity),code,movxy,movz,velrhop,TimeStep);
      }else{
...
//==============================================================================   
void ComputeStepSymplecticCor_ZhanCorr(bool floating,bool shift,bool inout,unsigned np,unsigned npb
  ,const float4 *velrhoppre,const float *ar,const float4 *shiftposfs
  ,const float3 *indirvel,double dtm,double dt,float rhopzero,float rhopoutmin,float rhopoutmax
  ,typecode *code,double2 *movxy,double *movz,float4 *velrhop,cudaStream_t stm)
{
  if(np){
    ...
    if(inout){      const bool tinout=true;
      if(shift){    const bool shift=true;
        if(floating)KerComputeStepSymplecticCor_ZhanCorr<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,code,movxy,movz,velrhop);
        else        KerComputeStepSymplecticCor_ZhanCorr<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,code,movxy,movz,velrhop);
      }else{        const bool shift=false;
        if(floating)KerComputeStepSymplecticCor_ZhanCorr<true ,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,code,movxy,movz,velrhop);
        else        KerComputeStepSymplecticCor_ZhanCorr<false,shift,tinout> <<<sgrid,SPHBSIZE,0,stm>>> (np,npb,velrhoppre,ar,shiftposfs,indirvel,dtm,dt,rhopzero,rhopoutmin,rhopoutmax,code,movxy,movz,velrhop);
      }
