...
///Structure Phase Array holds hypoplastic physical proprerties of phases
typedef struct {
    word mkfluid;				///<Mk of the phase
    int phaseid;		///<ID of the phase
    unsigned idbegin;		///<First id of phase.
    ...
    float visco;		///<viscosity of the phase. 
  //==================================================
  // hypoplastic model constitutive parameters 
  //==================================================
    float Hypo_angle; ///<  Internal friction angle
    float Hypo_hs;    ///< hs
    float Hypo_n;    ///< n
    float Hypo_ed0;    ///< ed_0
    float Hypo_ec0;    ///< ec_0
    float Hypo_ei0;    ///< ei_0 
    float Hypo_alpha;    ///< alpha
    float Hypo_beta;    ///< beta
    float Hypo_wallfriction; ///miu wall friction
    tfloat3 Hypo_initStres; ///
    float Hypo_voidRatio; ///initial void ratio
    float Hypo_rhoparticle;
    unsigned phasetype; ///<Typer of phase
}StPhaseHypo;
//<vs_non-Newtonian_end>


///Structure Phase Array holds elastic physical proprerties of phases
typedef struct {
    word mkfluid;				///<Mk of the phase
    int phaseid;		///<ID of the phase
    unsigned idbegin;		///<First id of phase.
    ...
    float visco;		///<viscosity of the phase. 
  //==================================================
  // elastic model constitutive parameters 
  //==================================================
    float lameparm1; ///<  first lame parameter, lambda
    float lameparm2;    ///< second lame parameter, mu
    tfloat3 Elastic_initStres; /// give a small initial stress
    float Elastic_wallfriction; ///miu wall friction
    unsigned phasetype; ///<Typer of phase
}StPhaseElastic;
...
typedef enum{ 
  ...
  VISCO_Hypoplasticity = 4,  ///hypoplastic model
  VISCO_Elasticity = 5,      ///elastic model
  VISCO_None=0 
}TpVisco;            
...
///Types of boundary conditions. 
typedef enum{
  SLIP_FreeSlip_Zhan=7,
  SLIP_NoSlip_Zhan=6, 
  SLIP_Friction_Zhan=5, /// Frictional slip (Zhan et al., 2020)
  SLIP_Friction_Soleimani=4,   ///Frictonal slip (Soleimani et al., 2021)
