...
typedef struct{
  ...
  tsymatrix3f *spstau;
  tfloat3 *Force;
  tmatrix3f *spsgradvel; ///swithched from sysmetric to full tensor
  //<vs_non-Newtonian_ini>
  float *visco_eta; 
  float *void_ratio;
  tsymatrix3f *d_tensor;
  tfloat3 *w_tensor;   ///hypoplastic model need consider spin tensor
  float *auxnn; 
  tfloat3* BoundNormalc;
  bool *BoundCornerc; /// Denote whether a bound particle is at the corner or not
...
inline stinterparmsc StInterparmsc(unsigned np,unsigned npb,unsigned npbok
  ...
  ,const tdouble3 *pos,tfloat4 *velrhop,const unsigned *idp,const typecode *code
  ,const float *press
  ,float* ar,tfloat3 *ace,float *delta
  ,TpShifting shiftmode,tfloat4 *shiftposfs
  ,tsymatrix3f *spstau,tfloat3 *Force,tmatrix3f *spsgradvel
  ,float* visco_eta, float *void_ratio, tsymatrix3f *d_tensor, tfloat3 *w_tensor, float *auxnn, tfloat3 *BoundNormalc, bool *BoundCornerc //<vs_non-Newtonian>
)
{
  stinterparmsc d={np,npb,npbok,(np-npb)
    ...
    ,spstau,Force,spsgradvel
    ,visco_eta,void_ratio,d_tensor,w_tensor,auxnn,BoundNormalc,BoundCornerc  //<vs_non-Newtonian>
  };
  return(d);
}
...
protected:
  ...
  tfloat3 *Forcec;            /// Reaction force applied from soil particles to the boundary particles
  ///swithched from sysmetric to full tensorr
  tmatrix3f *SpsGradvelc;   ///<Velocity gradients.  

  TimersCpu Timers;


// additional varibles to be use for hypoplasticity modeling
//----------------------------------
  // tsymatrix3f *SigmaM1c; ///<Verlet: previous values of the stress tensor/may be replaced with SpsTauc
  tfloat3 *W_tensorc; ///spin tensor
  float *Void_ratioc; ///void ratio 
//----------------------------------
  ...
  void PreInteractionVars_Forces(unsigned np,unsigned npb,TpInterStep interstep, int Zhan_loop);
  void PreInteraction_Forces(TpInterStep interstep, int Zhan_loop);
  void PosInteraction_Forces(bool ReserveArc);
  ...
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift> 
    void Interaction_ForcesCpuT(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity> void Interaction_Forces_ct5(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco> void Interaction_Forces_ct4(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  template<TpKernel tker,TpFtMode ftmode> void Interaction_Forces_ct3(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  template<TpKernel tker> void Interaction_Forces_ct2(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  void Interaction_Forces_ct(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  ...
  void ComputeSpsTau(unsigned n,unsigned pini,const tfloat4 *velrhop,const tmatrix3f *gradvel,tsymatrix3f *tau)const;
  ...
  void ComputeSymplecticCorr_ZhanPre(double dt);
  void ComputeSymplecticCorr_ZhanCorr(double dt);
...
protected:
  ...
  unsigned GetParticlesData(unsigned n,unsigned pini,bool onlynormal
    ,unsigned *idp,tdouble3 *pos,tfloat3 *vel,float *rhop,typecode *code
    ,float *aux_n, tfloat3 *stress, float *void_ratio, tfloat3 *Force);
...
//==============================================================================
/// declarification of functions for hypopoplasticity and elasticity model
//==============================================================================
  void GetStrainRateSpinTensors_tsym(const tmatrix3f &dvelp1,tsymatrix3f &D_tensor, tfloat3 &W_tensor)const;
  void GetStressTensor_tsym_hypo(const tsymatrix3f &D_vector, const tfloat3 &W_vector,
      const tsymatrix3f &SigmaOld_vector, float &voidRatio, const double &dt,
      tsymatrix3f &tau_vector,  const float &Hypo_angle, const float &Hypo_hs,
      const float &Hypo_n, const float &Hypo_ed0, const float &Hypo_ec0,
      const float &Hypo_ei0, const float &Hypo_alpha, const float &Hypo_beta, const unsigned p1)const;
  void GetStressTensor_tsym_elastic(const tsymatrix3f &D_vector, const tfloat3 &W_vector,
      const tsymatrix3f &SigmaOld_vector, const double &dt,tsymatrix3f &tau_vector, const float &lameparm2, const float &lameparm1, const unsigned p1)const;
 template<TpKernel tker> void StressRegularization_SPH (unsigned n,unsigned pinit,StDivDataCpu divdata,const unsigned *dcell
  ,tsymatrix3f *tau, const tdouble3 *pos,const tfloat4 *velrhop,const typecode *code,const TpVisco TVisco)const;
  void StressRegularization_SPH_ct(unsigned n,unsigned pinit,StDivDataCpu divdata,const unsigned *dcell
  ,tsymatrix3f *tau, const tdouble3 *pos,const tfloat4 *velrhop,const typecode *code,const TpVisco TVisco)const;
  //SPH Symetric
  ...
  void GetStrainRateTensor_tsym(const tmatrix3f &dvelp1
    ,float &I_D,float &II_D,float &J1_D,float &J2_D,float &div_D_tensor
  ...
  (unsigned n,unsigned pini,StDivDataCpu divdata,const unsigned *dcell
    ,const tdouble3 *pos,const tfloat4 *velrhop,const typecode *code,const unsigned *id
    ,float &viscdt,float *ar)const;
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco> void InteractionForcesBound_NN_SPH
  (unsigned n,unsigned pini,StDivDataCpu divdata,const unsigned *dcell
    ,tsymatrix3f *tau, const tdouble3 *pos,tfloat4 *velrhop,const typecode *code,const unsigned *id
  ...
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift>
  void InteractionForcesFluid_NN_SPH_ConsEq_Zhan_bound(unsigned n,unsigned pini,bool boundp2,float visco
    ,StDivDataCpu divdata,const unsigned *dcell
  ...
  (unsigned n,unsigned pinit,float visco,float *visco_eta
    ,float *void_ratio, tsymatrix3f* tau,const tmatrix3f* gradvel,const tsymatrix3f* D_tensor,const tfloat3* W_tensor, 
    const tfloat4 *velrhop,float *auxnn,const tdouble3 *pos,const typecode *code,
    const unsigned *idp, double &dt)const;
  ...
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift>
  void Interaction_ForcesCpuT_NN_FDA(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;

  //FDA or SPH
  template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift>
  void Interaction_ForcesCpuT_NN_SPH(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const;
  //<vs_non-Newtonian_end>
