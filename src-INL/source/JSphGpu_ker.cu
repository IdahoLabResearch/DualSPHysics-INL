...
//------------------------------------------------------------------------------
template<bool periactive> __device__ void KerUpdatePos_CorrZhanPre
  (double movx,double movy,double movz,bool outrhop,unsigned p,double2 *posxy,double *posz,unsigned *dcell,typecode *code)
{
  //-Checks validity of displacement. | Comprueba validez del desplazamiento.
  const bool outmove=(fmaxf(fabsf(float(movx)),fmaxf(fabsf(float(movy)),fabsf(float(movz))))>CTE.movlimit);
  //-Applies diplacement.
  double3 rpos=make_double3(posxy[p].x,posxy[p].y,posz[p]);
  rpos.x+=movx; rpos.y+=movy; rpos.z+=movz;
  if(rpos.y<0 && CTE.symmetry)rpos.y=-rpos.y; //<vs_syymmetry>
  //-Checks limits of real domain. | Comprueba limites del dominio reales.
  double dx=rpos.x-CTE.maprealposminx;
  ...
  posxy[p].x=rpos.x; posxy[p].y=rpos.y; posz[p]=rpos.z;
}
...
__global__ void KerComputeSpsTau(unsigned n,unsigned pini,float smag,float blin
  ,const float4 *velrhop,const float3 *gradvelff,float2 *tauff)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; 
  if(p<n){
    const unsigned p1=p+pini;
    float3 rr1=gradvelff[p1*3];   
    float3 rr2=gradvelff[p1*3+1]; 
    float3 rr3=gradvelff[p1*3+2]; 
    const float grad_xx=rr1.x,grad_xy=0.5*(rr1.y+rr2.x);
    const float grad_xz=0.5*(rr1.z+rr3.x),grad_yy=rr2.y;
    const float grad_yz=0.5*(rr2.z+rr3.y),grad_zz=rr3.z;
...
__global__ void KerInitializeSpsTauVoidForce(unsigned n,unsigned pini,const typecode *code
  ,float2 *tauff, float *void_ratio,float3 *force,TpVisco tvisco)
{
  ...
  if(p<n){
    ...
    if(tvisco==VISCO_Hypoplasticity) void_ratio[p1] = PHASEHYPO[pp1].Hypo_voidRatio;
    tfloat3 stress_init=(tvisco==VISCO_Hypoplasticity ? PHASEHYPO[pp1].Hypo_initStres : PHASEELASTIC[pp1].Elastic_initStres);
    const float tau_xx=stress_init.x;
    const float tau_yy=stress_init.y;
    const float tau_zz=stress_init.z; 
    tauff[p1*3]=make_float2(tau_xx,0);
    tauff[p1*3+1]=make_float2(0,tau_yy);
    tauff[p1*3+2]=make_float2(0,tau_zz);
    force[p1]=make_float3(0,0,0);
  }
}

__global__ void KerDownloadSpsTau(unsigned n,unsigned pini
  ,float2 *tauff, float3 *sigma)
{
  unsigned p=blockIdx.x*blockDim.x + threadIdx.x; 
  if(p<n){
    const unsigned p1=p+pini;
    const float2 taup1_xx_xy=tauff[p1*3];
    const float2 taup1_xz_yy=tauff[p1*3+1];
    const float2 taup1_yz_zz=tauff[p1*3+2];
    sigma[p1]=make_float3(taup1_xx_xy.x,taup1_xz_yy.y,taup1_yz_zz.y);
  }
}
...
void ComputeSpsTau(unsigned np,unsigned npb,float smag,float blin
  ,const float4 *velrhop,const tmatrix3f *gradvelg,tsymatrix3f *tau,cudaStream_t stm)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetSimpleGridSize(npf,SPHBSIZE);
    KerComputeSpsTau <<<sgridf,SPHBSIZE,0,stm>>> (npf,npb,smag,blin,velrhop,(const float3*)gradvelg,(float2*)tau);
  }
}

void InitializeSpsTauVoidForce(unsigned np,unsigned npb,const typecode *code, tsymatrix3f *tau, float *Void_ratiog, float3 *Forceg,TpVisco tvisco, cudaStream_t stm)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetSimpleGridSize(npf,SPHBSIZE);
...
void DownloadSpsTau(unsigned np,unsigned npb,const tsymatrix3f *tau,float3 *Sigmag, cudaStream_t stm)
{
  const unsigned npf=np-npb;
  if(npf){
    dim3 sgridf=GetSimpleGridSize(npf,SPHBSIZE);
    KerDownloadSpsTau <<<sgridf,SPHBSIZE,0,stm>>> (npf,npb,(float2*)tau, Sigmag);
  }
}
...
//------------------------------------------------------------------------------
/// Updates particle position according to displacement. Corr_zhanpre step
//------------------------------------------------------------------------------
template<bool periactive,bool floatings> __global__ void KerComputeStepPos2_CorrZhanPre(unsigned n,unsigned pini
  ,const double2 *movxy,const double *movz,double2 *posxy,double *posz,unsigned *dcell,typecode *code)
{
  unsigned pt=blockIdx.x*blockDim.x + threadIdx.x; //-Number of particle.
  if(pt<n){
    unsigned p=pt+pini;
    ...
    if(normal){//-Does not apply to periodic particles. | No se aplica a particulas periodicas
      if(fluid){//-Only applied for fluid displacement. | Solo se aplica desplazamiento al fluido.
        const double2 rmovxy=movxy[p];
        KerUpdatePos_CorrZhanPre<periactive>(rmovxy.x,rmovxy.y,movz[p],outrhop,p,posxy,posz,dcell,code);
      }
    }
  }
}
...
//==============================================================================
/// Updates particle position according to displacement. corre_zhan_pre step
//==============================================================================
void ComputeStepPos2_CorrZhanPre(byte periactive,bool floatings,unsigned np,unsigned npb
  ,const double2 *movxy,const double *movz,double2 *posxy,double *posz,unsigned *dcell,typecode *code)
{
  const unsigned pini=npb;
  const unsigned npf=np-pini;
  if(npf){
    dim3 sgrid=GetSimpleGridSize(npf,SPHBSIZE);
    if(periactive){ const bool peri=true;
      if(floatings)KerComputeStepPos2_CorrZhanPre<peri,true>  <<<sgrid,SPHBSIZE>>> (npf,pini,movxy,movz,posxy,posz,dcell,code);
      else         KerComputeStepPos2_CorrZhanPre<peri,false> <<<sgrid,SPHBSIZE>>> (npf,pini,movxy,movz,posxy,posz,dcell,code);
    }
    else{ const bool peri=false;
      if(floatings)KerComputeStepPos2_CorrZhanPre<peri,true>  <<<sgrid,SPHBSIZE>>> (npf,pini,movxy,movz,posxy,posz,dcell,code);
      else         KerComputeStepPos2_CorrZhanPre<peri,false> <<<sgrid,SPHBSIZE>>> (npf,pini,movxy,movz,posxy,posz,dcell,code);
