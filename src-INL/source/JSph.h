...
protected:
  ...
  TpSlipMode SlipMode;        ///<Slip mode for mDBC 1:DBC vel=0, 2:No-slip, 3:Free slip. 4: frictional slip - Soleimani method. 5: frictional slip - Zhan method. 6:No slip - Zhan method. 7: Free slip - Zhan method (default=1).
  ...
  int Nregularization;     ///<Number of step for stress regularization
  bool regularizationSwitch; ///<Control on and off of regularization 
  ...
  double Time_inc;        // time increment stored for stress integration
...
protected:
  ...
  void LoadBoundNormals(unsigned np,unsigned npb,const unsigned *idp,const typecode *code,tfloat3 *boundnormal,bool *BoundCorner);
  void ConfigBoundNormals(unsigned np,unsigned npb,const tdouble3 *pos,const unsigned *idp,tfloat3 *boundnormal, bool *BoundCorner);
...
protected:
  ...
  StPhaseHypo* PhaseHypo;     ///contains hypoplasticity phase material parameters and initial stress condition
  StPhaseElastic* PhaseElastic;     ///contains elasticity phase material parameters

  void InitMultiPhase(const JXml *sxml,std::string xmlpath);
  void InitHypoPhase(const JXml *sxml,std::string xmlpath);
  void InitElasticPhase(const JXml *sxml,std::string xmlpath);
  void ConfigConstantsMP();
  void ConfigConstantsMP_Hypo();
  void ConfigConstantsMP_Elastic();
