...
protected:
  ...
  float *Void_ratio; //void ratio at host
  tfloat3 *Sigma;      //stress diagonal component in host for output
  // additional varibles to be use for hypoplasticity modeling and elasticity 
  tfloat3 *Force;
  ...
  tmatrix3f *SpsGradvelg;     ///<Velocity gradients.
...
//----------------------------------
  ...
  float3 *W_tensorg; ///spin tensor
  float *Void_ratiog; ///void ratio at GPU memeory
  float *Distance_ratiog;
//----------------------------------
  ...
  void ConstantDataUp();
  void ParticlesDataUp(unsigned n,const tfloat3 *boundnormal,const bool *BoundCorner);
  ...
  void PreInteractionVars_Forces(unsigned np,unsigned npb,TpInterStep interstep, int Zhan_loop,tsymatrix3f *SpsTaug,float *Pressg);
  void PreInteraction_Forces(TpInterStep interstep, int Zhan_loop, tsymatrix3f *SpsTaug);
