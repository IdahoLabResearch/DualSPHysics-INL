...
JSphGpu::JSphGpu(bool withmpi):JSph(false,false,withmpi),DivAxis(MGDIV_None){
  ...
  Void_ratio =NULL; Sigma = NULL; //granular model state varibles for output
  CellDiv=NULL;
...
void JSphGpu::InitVars(){
  ...
  BoundNormalg=NULL; MotionVelg=NULL; BoundCornerg=NULL;//-mDBC
  ...
  W_tensorg=NULL; Void_ratiog= NULL;             //Granular material spin tensor and void ratio
  Distance_ratiog = NULL;
  Sigmag=NULL;                                   //store stress component for downloading to host
...
void JSphGpu::FreeCpuMemoryParticles(){
  ...
  delete[] Void_ratio; Void_ratio=NULL; //hypoplastic model
...
void JSphGpu::AllocCpuMemoryParticles(unsigned np){
  ...
  if(np>0){
    try{
      ...
      Void_ratio=new float[np];  MemCpuParticles+=sizeof(float)*np; //hypomodel
      Sigma=new tfloat3[np];     MemCpuParticles+=sizeof(tfloat3)*np;
      Force=new tfloat3[np];     MemCpuParticles+=sizeof(tfloat3)*np;
    }
    catch(const std::bad_alloc){
...
void JSphGpu::AllocGpuMemoryParticles(unsigned np,float over){
  ...
  // Need come debugging and checking
   //<vs_non-Newtonian for graular model (e.g. hypoplasticity)>
  if (TVisco != VISCO_Artificial) { //<vs_non-Newtonian>   
	  ArraysGpu->AddArrayCount(JArraysGpu::SIZE_24B,1); //-SpsTau
    // switched from sysmetric matrix to full matrix
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_36B,1); //-SpsGradvel
  }
  if (MultiPhase) { 			//<vs_non-Newtonian> 
    Log->Printf("ViscoTreatment= %u ",TVisco);
        // <hypoplasticity>
    if(TVisco==VISCO_Hypoplasticity){
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,3);//-w_tensorg, Sigmag,Forceg
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_24B,1); //d_tensorg
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,2); //-Visco_etag,Visco_etaDt
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,2); //-Void_ratiog, Distance_ratiog
    }else if(TVisco==VISCO_Elasticity){
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_12B,3);//-w_tensorg, Sigmag,Forceg
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_24B,1); //d_tensorg
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,2); //-Visco_etag,Visco_etaDt
      ArraysGpu->AddArrayCount(JArraysGpu::SIZE_4B,1); //-Distance_ratiog
    }
    else{
  ...
  if(UseNormals){
    ...
    ArraysGpu->AddArrayCount(JArraysGpu::SIZE_1B,1); //-BoundCorner
...
void JSphGpu::ResizeGpuMemoryParticles(unsigned npnew){
  ...
  float3      *force      =SaveArrayGpu(Np,Forceg);
  float3      *boundnormal=SaveArrayGpu(Np,BoundNormalg);
  bool        *BoundCorner=SaveArrayGpu(Np,BoundCornerg);
  ...
  ArraysGpu->Free(Forceg);
  ArraysGpu->Free(BoundNormalg);
  ArraysGpu->Free(BoundCornerg);
  ...
  if(force)      Forceg      =ArraysGpu->ReserveFloat3();
  if(boundnormal)BoundNormalg=ArraysGpu->ReserveFloat3();
  if(BoundCorner)BoundCornerg=ArraysGpu->ReserveBool();
  ...
  RestoreArrayGpu(Np,force,Forceg);
  RestoreArrayGpu(Np,boundnormal,BoundNormalg);
  RestoreArrayGpu(Np,BoundCorner,BoundCornerg);
...
void JSphGpu::ReserveBasicArraysGpu(){
  ...
  //check here
  if(MultiPhase && TVisco!=VISCO_Hypoplasticity && TVisco!=VISCO_Elasticity)AuxNNg=ArraysGpu->ReserveFloat();  //<vs_non-Newtonian>
  if((!MultiPhase && TVisco==VISCO_LaminarSPS) || (TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity)){
    // printf("mark-7\n");
    SpsTaug=ArraysGpu->ReserveSymatrix3f();  //<vs_non-Newtonian>/hypoplastic model
    Sigmag = ArraysGpu->ReserveFloat3(); //This is used for stress output
    Forceg=ArraysGpu->ReserveFloat3();
  }
  if(TVisco==VISCO_Hypoplasticity) Void_ratiog=ArraysGpu->ReserveFloat(); //hypoplastic model 
  if(UseNormals){
    BoundNormalg=ArraysGpu->ReserveFloat3();
    BoundCornerg=ArraysGpu->ReserveBool();
...
void JSphGpu::ConstantDataUp(){
  ...
  ctes.gravity.x=Gravity.x;ctes.gravity.y=Gravity.y;ctes.gravity.z=Gravity.z;
  cusph::CteInteractionUp(&ctes);
  //<vs_non-Newtonian>
  if(MultiPhase){
    if(TVisco == VISCO_Hypoplasticity){
      cusphNN::CteInteractionUp_NN(PhaseCount,PhaseHypo);
    }else if(TVisco == VISCO_Elasticity){
      cusphNN::CteInteractionUp_NN(PhaseCount,PhaseElastic);
    }else{
      cusphNN::CteInteractionUp_NN(PhaseCount,PhaseCte,PhaseArray);
    }
  }
...
//==============================================================================
void JSphGpu::ParticlesDataUp(unsigned n,const tfloat3 *boundnormal,const bool *BoundCorner){
  ...
  if(UseNormals){
    cudaMemcpy(BoundNormalg,boundnormal,sizeof(float3)*n,cudaMemcpyHostToDevice);
    cudaMemcpy(BoundCornerg,BoundCorner,sizeof(bool)*n,cudaMemcpyHostToDevice);
    }
  if(MultiPhase && TVisco!=VISCO_Hypoplasticity && TVisco!=VISCO_Elasticity)cudaMemcpy(AuxNNg,AuxNN,sizeof(float)*n,cudaMemcpyHostToDevice); //<vs_non-Newtonian>
  if(TVisco==VISCO_Hypoplasticity){
    cudaMemcpy(Forceg   ,Force   ,sizeof(float3)*n  ,cudaMemcpyHostToDevice);
    cudaMemcpy(Void_ratiog,Void_ratio,sizeof(float)*n  ,cudaMemcpyHostToDevice);
  }
  if(TVisco==VISCO_Elasticity) cudaMemcpy(Forceg   ,Force   ,sizeof(float3)*n  ,cudaMemcpyHostToDevice);
  //jinw@void ratio and stress tensor state variables need to be transfered to GPU memory  
  // if((!MultiPhase && TVisco==VISCO_LaminarSPS) || TVisco == VISCO_Hypoplasticity){
  //   cudaMemcpy(SpsTaug,SpsTau,sizeof(tsymatrix3f)*n,cudaMemcpyHostToDevice);
  // }
...
unsigned JSphGpu::ParticlesDataDown(unsigned n,unsigned pini,bool code,bool onlynormal){
  ...
  if(TVisco==VISCO_Hypoplasticity){
    cudaMemcpy(Void_ratio,Void_ratiog+pini,sizeof(float)*n,cudaMemcpyDeviceToHost);
    ...
    for(unsigned p=0;p<n;p++){
      ...
      if(ndel && normal){
        ...
        if (TVisco==VISCO_Hypoplasticity || TVisco==VISCO_Elasticity){
          if (TVisco==VISCO_Hypoplasticity) Void_ratio[p-ndel]=Void_ratio[p];
  ...
  if(CellMode==CELLMODE_Full || CellMode==CELLMODE_Half){
    ...
    double dt=0;
      ...
      const StInterParmsg parms=StrInterParmsg(Simulate2D
        ...
        ,TKernel,FtMode
        ,lamsps,MultiPhase,TVisco, TVelGrad,TDensity,ShiftingMode,SlipMode
        ,0,0,0,0,100,0,0
        ,0,divdatag,NULL
        ,NULL,NULL,NULL
        ,NULL,NULL,NULL
        ...
        ,NULL
        ,NULL,NULL
        ,VelMax,Pressg,Forceg,BoundNormalg,BoundCornerg,false,NULL,&kerinfo);
      if(!MultiPhase)cusph::Interaction_Forces(parms);
      else {
        int Zhan_loop = 0;
        cusphNN::Interaction_ForcesNN(parms,Time_inc,Zhan_loop);
      }
...
void JSphGpu::InitRunGpu(){
  ...
  if((!MultiPhase && TVisco==VISCO_LaminarSPS)|| (TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity)){
    ...
    if(TVisco == VISCO_Hypoplasticity) cudaMemset(Void_ratiog,0,sizeof(float)*Np);
  }
  if(TVisco == VISCO_Hypoplasticity || TVisco == VISCO_Elasticity){
    // Do the same as in CPU code "for (unsigned p=0; p<Np; p++){}"
    cusph::InitializeSpsTauVoidForce(Np,Npb,Codeg,SpsTaug,Void_ratiog,Forceg,TVisco);
  }
...
//==============================================================================
void JSphGpu::PreInteractionVars_Forces(unsigned np,unsigned npb,TpInterStep interstep, int Zhan_loop,tsymatrix3f *SpsTaug,float *Pressg){
  //-Initialise arrays.
  ...
  if(Forceg)cudaMemset(Forceg,0,sizeof(tfloat3)*np);                     //Forceg[]=(0,0,0)
  ...
  if(SpsGradvelg)cudaMemset(SpsGradvelg,0,sizeof(tmatrix3f)*np);    //SpsGradvelg[]=(0,0,0,0,0,0,0,0,0). changed to full size tensor form
  ...
  if(W_tensorg)cudaMemset(W_tensorg,0, sizeof(tfloat3)*np);
  if(Distance_ratiog)cudaMemset(Distance_ratiog,0, sizeof(float)*np);
  ...
  // To be changed to GPU code 
  //-Prepare press values for interaction.
  // Only code for hypo and elastic cases. Others not capable now.
  if(MultiPhase) {  //<vs_non-Newtonian>
    // hypoplasticity formulation use the whole stress tensor to update acceleration
    if (TVisco == VISCO_Hypoplasticity || TVisco == VISCO_Elasticity){
      cusphNN::ComputePress_NN(np,npb,SpsTaug,Pressg);  //<vs_non-Newtonian> 
    }
  // To be changed to GPU code above
  }
}
...
/// Prepara variables para interaccion.
//==============================================================================
void JSphGpu::PreInteraction_Forces(TpInterStep interstep, int Zhan_loop, tsymatrix3f *SpsTaug){
  ...
  if((interstep!=INTERSTEP_SymCorrector) || (Zhan_loop!=2)) Arg=ArraysGpu->ReserveFloat();
  ...
  if(TVisco!=VISCO_Artificial)SpsGradvelg=ArraysGpu->ReserveMatrix3f();//turned sysmatric to full matrix
  if(MultiPhase) {//<vs_non-Newtonian>
    if ((TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity)){
      D_tensorg = ArraysGpu->ReserveSymatrix3f();
      W_tensorg = ArraysGpu->ReserveFloat3();
      Distance_ratiog = ArraysGpu->ReserveFloat();
  ...
  //-Initialise arrays.
  PreInteractionVars_Forces(Np,Npb,interstep,Zhan_loop,SpsTaug,Pressg);
...
//==============================================================================
void JSphGpu::PosInteraction_Forces(bool ReserveArg){
  ...
  if(MultiPhase){
    ...
    ArraysGpu->Free(W_tensorg);	 W_tensorg=NULL;
    ArraysGpu->Free(Distance_ratiog); Distance_ratiog=NULL;
    ArraysGpu->Free(Visco_etag);	 Visco_etag=NULL;
    ArraysGpu->Free(ViscEtaDtg);	 ViscEtaDtg=NULL;
    if ((TVisco != VISCO_Hypoplasticity) && (TVisco != VISCO_Elasticity)) {
      ArraysGpu->Free(SpsTaug);      SpsTaug = NULL;
      ArraysGpu->Free(Void_ratiog);  Void_ratiog = NULL;
    }
  }
}
...
void JSphGpu::ComputeSymplecticPre(double dt){
  ...
  cusphs::ComputeStepSymplecticPre(WithFloating,shift,inout,Np,Npb,VelrhopPreg,Arg
    ...
    ,Codeg,movxyg,movzg,Velrhopg,NULL,TimeStep);
  //-Applies displacement to non-periodic fluid particles.
  //-Aplica desplazamiento a las particulas fluid no periodicas.
...
//==============================================================================
void JSphGpu::ComputeSymplecticCorr_ZhanPre(double dt){
 TmgStart(Timers,TMG_SuComputeStep);
  const bool shift=false; //(ShiftingMode!=SHIFT_None); //-We strongly recommend running the shifting correction only for the corrector. If you want to re-enable shifting in the predictor, change the value here to "true".
  //const bool WithFloating = false; //This code does not include floating implementation yet
  const bool inout=(InOut!=NULL);
  //PosxyPreg, PoszPreg, VelrhopPreg: n step, save, unchange
  //Posxyg, Poszg, Velrhopg: n+1/2 step, need to update to n + 1 predictor step
  const double dt05=dt*.5;
  //-Allocate memory to compute the diplacement.
  double2 *movxyg=ArraysGpu->ReserveDouble2();
  double *movzg=ArraysGpu->ReserveDouble();
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticCorr_ZhanPre(WithFloating,shift,inout,Np,Npb,Velrhopg
    ,Aceg,ShiftPosfsg,indirvel,dt05,Gravity,Codeg,movxyg,movzg,NULL,TimeStep);
  //-Applies displacement to non-periodic fluid particles.
  //-Aplica desplazamiento a las particulas fluid no periodicas.
  cusph::ComputeStepPos2_CorrZhanPre(PeriActive,WithFloating,Np,Npb,movxyg,movzg
  ,Posxyg,Poszg,Dcellg,Codeg);
  //-Frees memory allocated for the displacement
  ArraysGpu->Free(movxyg);   movxyg=NULL;
  ArraysGpu->Free(movzg);    movzg=NULL;
  TmgStop(Timers,TMG_SuComputeStep);
...
//==============================================================================
void JSphGpu::ComputeSymplecticCorr_ZhanCorr(double dt){
  TmgStart(Timers,TMG_SuComputeStep);
  const bool shift=(ShiftingMode!=SHIFT_None);
  const bool inout=(InOut!=NULL);
  //-Allocates memory to calculate the displacement.
  ...
  const double dt05=dt*.5;
  const float3 *indirvel=(InOut? InOut->GetDirVelg(): NULL);
  cusphs::ComputeStepSymplecticCor_ZhanCorr(WithFloating,shift,inout,Np,Npb,VelrhopPreg
    ,Arg,ShiftPosfsg,indirvel,dt05,dt,RhopZero,RhopOutMin,RhopOutMax
...
double JSphGpu::DtVariable(bool final){
  ...
  double dt;
  if ((TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity)){ //jinw@need check the computation steps
