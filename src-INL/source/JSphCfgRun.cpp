...
void JSphCfgRun::LoadOpts(string *optlis,int optn,int lv,const std::string &file){
  ...
  for(int c=0;c<optn;c++){
    ...
    if(opt[0]!='-' && opt[0]!='#'){
      ...
      else if(txword=="MDBC_Friction_Soleimani"){ TBoundary=2; SlipMode=4; }
      else if(txword=="MDBC_Friction_Zhan"){ TBoundary=2; SlipMode=5; }
      else if(txword=="MDBC_NoSlip_Zhan"){ TBoundary=2; SlipMode=6; }
      else if(txword=="MDBC_FreeSlip_Zhan"){ TBoundary=2; SlipMode=7; }
