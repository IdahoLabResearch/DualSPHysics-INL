...
typedef struct{
  ...
  float cfl_number;         ///coefficient to multiply with dt
...
typedef struct StrInterParmsg{
 ...
 // const tsymatrix3f *tau;
  ...
  tsymatrix3f *tau;
  tmatrix3f *gradvel; ///swithched from sysmetric to full tensorr
  tsymatrix3f *d_tensor; //<vs_non-Newtonian>
  float3 *w_tensor;   ///hypoplastic model need consider spin tensor
  float *void_ratio;   // hypo model need void ratio as state variable
  ...
  StrInterParmsg(
    ...
    //<vs_non-Newtonian_end>		
    ,TpDensity tdensity_,TpShifting shiftmode_,TpSlipMode slipmode_
    ...
    ,float4 *velrhop_,const unsigned *idp_,const typecode *code_
    ,const float *ftomassp_
    ,float *viscdt_,float *viscetadt_,float* ar_,float3 *ace_,float *delta_
    //<vs_non-Newtonian>
    ,float* visco_eta_, tsymatrix3f *spstau_, tmatrix3f *spsgradvel_
    ,tsymatrix3f *d_tensor_, float *auxnn_
    ,float3* w_tensor_, float* void_ratio_
    ,float* distance_ratio_
    //<vs_non-Newtonian>  
    ,float4 *shiftposfs_,double VelMax_ , float *Pressg_, float3 *Forceg_, float3*BoundNormalg_, bool *BoundCornerg_, bool regularize_
    ,cudaStream_t stm_
    ,StKerInfo *kerinfo_)
  {
    //-Configuration options.
    ...
    lamsps=lamsps_; tdensity=tdensity_; shiftmode=shiftmode_; slipmode=slipmode_;
    //-Execution values.
    ...
    gradvel=spsgradvel_;
    shiftposfs=shiftposfs_; VelMax=VelMax_;
    //<vs_non-Newtonian>    
	  multiphase = multiphase_;  tvelgrad = tvelgrad_; tvisco = tvisco_;
	  visco_eta = visco_eta_; tau = spstau_; gradvel = spsgradvel_;
    d_tensor = d_tensor_; auxnn = auxnn_;
    w_tensor = w_tensor_; void_ratio = void_ratio_;
    distance_ratio = distance_ratio_;
	  viscetadt = viscetadt_; Pressg = Pressg_;	Forceg = Forceg_; BoundNormalg = BoundNormalg_; BoundCornerg = BoundCornerg_; 
...
void ComputeSpsTau(unsigned np,unsigned npb,float smag,float blin
  ,const float4 *velrhop,const tmatrix3f *gradvelg,tsymatrix3f *tau,cudaStream_t stm=NULL);

//-Kernels for initilize state variables for granular constitutive model.
void InitializeSpsTauVoidForce(unsigned np,unsigned npb,const typecode *code
  ,tsymatrix3f *tau,float *Void_ratiog,float3 *Forceg,TpVisco tvisco, cudaStream_t stm=NULL);

void DownloadSpsTau(unsigned np,unsigned npb
  ,const tsymatrix3f *tau,float3 *Sigmag, cudaStream_t stm=NULL);
