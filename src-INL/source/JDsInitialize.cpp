...
/// Initialisation of variables.
//==============================================================================
void JDsInitializeOp_BoundNormalCurved::Reset() {
    JDsInitializeOp::Reset();
    MkBound = "";
    Center1 = TFloat3(0);
    Center2 = TFloat3(0);
...
/// Reads particles information in xml format.
//==============================================================================
void JDsInitializeOp_BoundNormalCurved::ReadXml(const JXml* sxml, TiXmlElement* xele) {
    sxml->CheckElementNames(xele, true, "onlypos center1 center2 inverse corner");
    ReadXmlOnlyPos(sxml, xele);
    ...
    Inverse = sxml->ReadElementBool(xele, "inverse","v");
    Corner  = sxml->ReadElementBool(xele, "corner", "v");
}

//==============================================================================
/// Initializes data of particles according XML configuration.
//==============================================================================
void JDsInitializeOp_BoundNormalCurved::Run(unsigned np, unsigned npb, const tdouble3* pos
    , const unsigned* idp, const word* mktype, tfloat4* velrhop, tfloat3* boundnormal, bool* BoundCorner)
{
    ...
    for (unsigned p = 0; p < np; p++)if (idp[p] < InitCt.nbound && (all || rg.CheckValue(mktype[p])) && CheckPos(p, pos)) {
        ...
        tdouble3 normalp = fgeo::VecUnitary(pcen - ps);
        if (Inverse) {
            normalp = fgeo::VecUnitary(ps - pcen);
        }
        boundnormal[p] = ToTFloat3(normalp);
        if (Corner) BoundCorner[p] = true;
    }
}
...
/// Returns strings with configuration.
//==============================================================================
void JDsInitializeOp_BoundNormalCurved::GetConfig(std::vector<std::string>& lines)const {
    lines.push_back(fun::PrintStr("  Operation: %s", ClassName.substr(BaseNameSize).c_str()));
    ...
    lines.push_back(fun::PrintStr("  Inverse...: %s", (Inverse ? "true" : "false")));
    lines.push_back(fun::PrintStr("  Corner...: %s", (Corner ? "true" : "false")));
}
...
void JDsInitializeOp_BoundNormalSet::ReadXml(const JXml *sxml,TiXmlElement* xele){
  sxml->CheckElementNames(xele,true,"onlypos normal corner");
  ReadXmlOnlyPos(sxml,xele);
  MkBound=sxml->GetAttributeStr(xele,"mkbound",true);
...
void JDsInitializeOp_BoundNormalPlane::ReadXml(const JXml *sxml,TiXmlElement* xele){
  sxml->CheckElementNames(xele,true,"onlypos point limitdist normal maxdisth corner");
  ReadXmlOnlyPos(sxml,xele);
  MkBound=sxml->GetAttributeStr(xele,"mkbound",true);
...
void JDsInitializeOp_BoundNormalSphere::ReadXml(const JXml *sxml,TiXmlElement* xele){
  sxml->CheckElementNames(xele,true,"onlypos center radius inside maxdisth corner");
  ReadXmlOnlyPos(sxml,xele);
  MkBound=sxml->GetAttributeStr(xele,"mkbound",true);
...
void JDsInitialize::ReadXml(const JXml *sxml,TiXmlElement* lis){
  ...
  while(ele){
    ...
    if(cmd.length() && cmd[0]!='_' && sxml->CheckElementActive(ele)){
      ...
      else if(cmd=="boundnormal_curved"  ){ if(BoundNormals){ JDsInitializeOp_BoundNormalCurved   *ope=new JDsInitializeOp_BoundNormalCurved  (sxml,ele,InitCt); Opes.push_back(ope); } }
  ...
  ,const unsigned *idp,const word *mktype,tfloat4 *velrhop,tfloat3 *boundnormal, bool *BoundCorner)
{
  for(unsigned c=0;c<Count();c++){
    Opes[c]->Run(np,npb,pos,idp,mktype,velrhop,boundnormal,BoundCorner);
  }
}
