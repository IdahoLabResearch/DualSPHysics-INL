...
void JSphCpu::InitVars(){
  ...
  BoundNormalc=NULL; MotionVelc=NULL; BoundCornerc=NULL; //-mDBC
...
// <hypoplasticity>
  W_tensorc=NULL;               //spin tensor
  Void_ratioc = NULL;           //void ratio
...
void JSphCpu::AllocCpuMemoryParticles(unsigned np,float over){
  ...
  if(TVisco!=VISCO_Artificial){ //<vs_non-Newtonian>   
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B,1); //-SpsTau
    // Log->Printf("mark-2");
    // switched from sysmetric matrix to full matrix
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_36B,1); //-SpsGradvel
  }
  if(MultiPhase){ //<vs_non-Newtonian>
      Log->Printf("ViscoTreatment= %u ",TVisco);
    // <hypoplasticity>
    if(TVisco==VISCO_Hypoplasticity){
      ArraysCpu->AddArrayCount(JArraysCpu::SIZE_12B,3);//-w_tensor,Force,Forcec
      ArraysCpu->AddArrayCount(JArraysCpu::SIZE_24B,1); //d_tensor
      ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B,1); //-Visco_etac
      ArraysCpu->AddArrayCount(JArraysCpu::SIZE_4B,2); //-void Ratios
  ...
  if(UseNormals){
    ...
    ArraysCpu->AddArrayCount(JArraysCpu::SIZE_1B,1); //-BoundCorner
...
void JSphCpu::ResizeCpuMemoryParticles(unsigned npnew){
  ...
  tfloat3     *Force      =SaveArrayCpu(Np,Forcec);
  tfloat3     *boundnormal=SaveArrayCpu(Np,BoundNormalc);
  bool        *BoundCorner=SaveArrayCpu(Np,BoundCornerc);
...
void JSphCpu::ReserveBasicArraysCpu(){
  ...
  // printf("mark-5\n");
  if(MultiPhase && TVisco!=VISCO_Hypoplasticity)AuxNN=ArraysCpu->ReserveFloat(); //<vs_non-Newtonian>
  // printf("mark-6\n");
  if((!MultiPhase && TVisco==VISCO_LaminarSPS) || (TVisco == VISCO_Hypoplasticity)|| (TVisco == VISCO_Elasticity)){
    // printf("mark-7\n");
    SpsTauc=ArraysCpu->ReserveSymatrix3f(); //<vs_non-Newtonian>/hypoplastic model
    Forcec=ArraysCpu->ReserveFloat3();
  }
  if(TVisco==VISCO_Hypoplasticity)Void_ratioc=ArraysCpu->ReserveFloat(); //hypoplastic model
  if(UseNormals){
    BoundNormalc=ArraysCpu->ReserveFloat3();
    BoundCornerc=ArraysCpu->ReserveBool();
...
unsigned JSphCpu::GetParticlesData(unsigned n,unsigned pini,bool onlynormal
  ...
  , float *aux_n, tfloat3 *Sigma, float *aux_void, tfloat3 *Force)
{
  unsigned num=n;
  ...
  if(aux_void) memcpy(aux_void ,Void_ratioc +pini,sizeof(float)*n);
  //<vs_non-Newtonian>
  // printf("mark--SaveData-2-00 \n");
  if(aux_n)memcpy(aux_n,AuxNN+pini,sizeof(float)*n);
  // printf("mark pini=%d \n", pini);
  if(vel && rhop){
    // printf("mark--SaveData-2-0 \n");
    for(unsigned p=0;p<n;p++){
      // printf("mark--SaveData-2-0 \n");
      // printf("mark Velrhopc.x=%f \n", Velrhopc[p+pini].x);
      ...
      if(Sigma){
        // printf("mark--SaveData-2-1: sig.x \f \n", SpsTauc[p+pini].xx);
        tsymatrix3f sigma = SpsTauc[p+pini];
        // printf("mark--SaveData-2-2 \n");
        // float sig_p = -(sigma.xx+sigma.yy+sigma.zz);
        // float sig_q = sqrt( 0.5*(pow((sigma.xx- sigma.yy),2)
        //               + pow((sigma.xx- sigma.zz),2)
        //               + pow((sigma.zz- sigma.yy),2)
        //               + 6*(pow(sigma.xy, 2) + pow(sigma.xz, 2) + pow(sigma.yz, 2))));
        // float J2 = 1/6*(pow((sigma.xx- sigma.yy),2)
        //               + pow((sigma.xx- sigma.zz),2)
        //               + pow((sigma.zz- sigma.yy),2)
        //               + pow(sigma.xy, 2) + pow(sigma.xz, 2) + pow(sigma.yz, 2));
        // float J3 = (-sigma.xx - sig_p)*(-sigma.yy - sig_p)*(-sigma.zz - sig_p)
        //           +2*sigma.xy*sigma.xz*sigma.yz
        //           -(-sigma.xx - sig_p)*pow(-sigma.yz, 2)
        //           -(-sigma.yy - sig_p)*pow(-sigma.xz, 2)
        //           -(-sigma.zz - sig_p)*pow(-sigma.xy, 2);
        // float sin3Theta = 0;
        // if (J2 == 0){
        //   sin3Theta =0;
        // }else{
        //   sin3Theta = 3*sqrt(3)/2 * J3/sqrt(pow(J2,3));
        // }
        // if (sin3Theta > 0.99) sin3Theta = 1;
        // if (sin3Theta < -0.99) sin3Theta = -1;
        // float  theta = 1./3. * asin(sin3Theta);
        // Sigma[p] = {sig_p,sig_q,theta};
        Sigma[p] = {sigma.xx, sigma.yy, sigma.zz};
      }
      if(Force){
        tfloat3 force = Forcec[p+pini];
        Force[p] = {force.x, force.y, force.z};
        //Force[p] = {1,2,3};
      }
      // printf("mark--SaveData-2-3 \n");
    }
  }
  else{
...
void JSphCpu::InitRunCpu(){
  ...
  if((!MultiPhase && TVisco==VISCO_LaminarSPS)|| (TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity)){
    memset(SpsTauc,0,sizeof(tsymatrix3f)*Np); //<vs_non-Newtonian>
    memset(Forcec,0,sizeof(tfloat3)*Np);
    if(TVisco == VISCO_Hypoplasticity) memset(Void_ratioc,0,sizeof(float)*Np);
    for (unsigned p=0 ; p<Np; p++){
      const typecode pp1=CODE_GetTypeValue(Codec[p]); //<vs_non-Newtonian>
      const tfloat3 stress_init=(TVisco == VISCO_Hypoplasticity? PhaseHypo[pp1].Hypo_initStres:PhaseElastic[pp1].Elastic_initStres);    
      SpsTauc[p].xx= stress_init.x;
      SpsTauc[p].yy= stress_init.y;
      SpsTauc[p].zz= stress_init.z;
      SpsTauc[p].xy= 0.0f;
      SpsTauc[p].xz= 0.0f;
      SpsTauc[p].yz= 0.0f;
      Forcec[p].x = 0.0f;
      Forcec[p].y = 0.0f;
      Forcec[p].z = 0.0f;
      if (TVisco == VISCO_Hypoplasticity) Void_ratioc[p] = PhaseHypo[pp1].Hypo_voidRatio;
    }
  }
...
//==============================================================================
void JSphCpu::PreInteractionVars_Forces(unsigned np,unsigned npb,TpInterStep interstep, int Zhan_loop){
  //-Initialise arrays.
  const unsigned npf=np-npb;
  if((interstep!=INTERSTEP_SymCorrector) || (Zhan_loop!=2)){
  ...
  if(Forcec)memset(Forcec,0,sizeof(tfloat3)*np);                     //Forcec[]=(0,0,0)
  //<vs_non-Newtonian>     
  // -SpsGradvelc changed from symetric matrix to full matrixin
  // printf("mark JSphCpu PreInteractionVars_Forces - 1");
  if(SpsGradvelc)memset(SpsGradvelc,0,sizeof(tmatrix3f)*np);  //SpsGradvelc[]=(0,0,0,0,0,0,0,0,0).
  // -we need to use the previous stress to update stress at this increment
  //  printf("mark JSphCpu PreInteractionVars_Forces - 2");
  if ((TVisco != VISCO_Hypoplasticity) && (TVisco != VISCO_Elasticity)){
    if(SpsTauc)  memset(SpsTauc,0,sizeof(tsymatrix3f)*np);
  }
  //  printf("mark JSphCpu PreInteractionVars_Forces - 3");
  ...
  // <hypoplasticity>
  //  printf("mark JSphCpu PreInteractionVars_Forces - 4");
  if(W_tensorc)  memset(W_tensorc,0, sizeof(tfloat3)*np);
  // if(Void_ratioc) memset(W_tensorc,0, sizeof(float)*np);
  //-Select particles for shifting.
  //  printf("mark JSphCpu PreInteractionVars_Forces - 5");
  if(ShiftPosfsc)Shifting->InitCpu(npf,npb,Posc,ShiftPosfsc);
  // printf("mark JSphCpu PreInteractionVars_Forces - 6");
  ...
  if(!MultiPhase) {  //<vs_non-Newtonian>
    ...
    // hypoplasticity formulation use the whole stress tensor to update acceleration
    // if (TVisco != VISCO_Hypoplasticity){
      ComputePress_NN(np,npb);  //<vs_non-Newtonian> 
    // }
  }
  // printf("mark JSphCpu PreInteractionVars_Forces - 7");
}
//==============================================================================
/// Prepare variables for interaction functions.
/// Prepara variables para interaccion.
//==============================================================================
void JSphCpu::PreInteraction_Forces(TpInterStep interstep, int Zhan_loop){
  ...
  // change the dradvelc from symetric matrix to full matrix
  // printf("mark in PreInteraction_Forces reserve for SpsGradvelc");
  if(TVisco!=VISCO_Artificial)SpsGradvelc=ArraysCpu->ReserveMatrix3f();
  // printf("mark in PreInteraction_Forces reserve for Dtensor");
  if(MultiPhase) {//<vs_non-Newtonian>
    if ((TVisco == VISCO_Hypoplasticity) ||(TVisco == VISCO_Elasticity)){
      D_tensorc=ArraysCpu->ReserveSymatrix3f();
      W_tensorc = ArraysCpu->ReserveFloat3();
      Visco_etac=ArraysCpu->ReserveFloat();
      //Forcec=ArraysCpu->ReserveFloat3();
      // Void_ratioc = ArraysCpu->ReserveFloat();
    }else{
      D_tensorc=ArraysCpu->ReserveSymatrix3f();
      Visco_etac=ArraysCpu->ReserveFloat();
  ...
  //-Initialise arrays & avoid to update pressure according to EOS
  PreInteractionVars_Forces(Np,Npb,interstep,Zhan_loop);
...
//==============================================================================
void JSphCpu::PosInteraction_Forces(bool ReserveArc){
  ...
  //ArraysCpu->Free(Forcec);       Forcec=NULL;
  //<vs_non-Newtonian>  
  ArraysCpu->Free(SpsGradvelc);  SpsGradvelc=NULL;
  // vn_non_Newtonian varaibles are not released
  ArraysCpu->Free(D_tensorc);    D_tensorc=NULL;
  ArraysCpu->Free(W_tensorc);    W_tensorc=NULL;
  ArraysCpu->Free(Visco_etac);   Visco_etac=NULL;
  // should not released for hypoplasticity or elasticity
  if ((MultiPhase) && (TVisco != VISCO_Hypoplasticity) && (TVisco != VISCO_Elasticity)) {
     ArraysCpu->Free(SpsTauc);      SpsTauc = NULL;
     ArraysCpu->Free(Void_ratioc);  Void_ratioc = NULL;
  }
  // should not released for hypoplasticity
  if ((MultiPhase) && (TVisco != VISCO_Hypoplasticity)) {
     ArraysCpu->Free(Void_ratioc);  Void_ratioc = NULL;
  }
  // printf("mark JSphCpu::PosInteraction_Forces \n");
}
...
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift> 
  ...
  ,const tsymatrix3f* tau,tmatrix3f* gradvel
  ,const tdouble3 *pos,const tfloat4 *velrhop,const typecode *code,const unsigned *idp
  ,const float *press 
  ...
  for(int p1=int(pinit);p1<pfin;p1++){
    ...
    if(shift||arp1||acep1.x||acep1.y||acep1.z||visc){
      ...
      if(tvisco==VISCO_LaminarSPS){
        gradvel[p1].a11+=gradvelp1.xx;
        gradvel[p1].a12+=gradvelp1.xy/2;
        gradvel[p1].a21+=gradvelp1.xy/2;
        gradvel[p1].a13+=gradvelp1.xz/2;
        gradvel[p1].a31+=gradvelp1.xz/2;
        gradvel[p1].a22+=gradvelp1.yy;
        gradvel[p1].a23+=gradvelp1.yz/2;
        gradvel[p1].a32+=gradvelp1.yz/2;
        gradvel[p1].a33+=gradvelp1.zz;
      }
...
//==============================================================================
void JSphCpu::ComputeSpsTau(unsigned n,unsigned pini,const tfloat4 *velrhop,const tmatrix3f *gradvel,tsymatrix3f *tau)const{
  const int pfin=int(pini+n);
  #ifdef OMP_USE
    #pragma omp parallel for schedule (static)
  #endif
  for(int p=int(pini);p<pfin;p++){
    const tmatrix3f gradvel=SpsGradvelc[p];
    const float pow1=gradvel.a11*gradvel.a11 + gradvel.a22*gradvel.a22 + gradvel.a33*gradvel.a33;
    const float prr=pow1+pow1 + 2*gradvel.a12*gradvel.a12 + 2*gradvel.a13*gradvel.a13 + 2*gradvel.a23*gradvel.a23;
    const float visc_sps=SpsSmag*sqrt(prr);
    const float div_u=gradvel.a11+gradvel.a22+gradvel.a33;
    ...
    tau[p].xx=one_rho2*(twovisc_sps*gradvel.a11 +sumsps);
    tau[p].xy=one_rho2*(visc_sps   *gradvel.a12*2);
    tau[p].xz=one_rho2*(visc_sps   *gradvel.a13*2);
    tau[p].yy=one_rho2*(twovisc_sps*gradvel.a22 +sumsps);
    tau[p].yz=one_rho2*(visc_sps   *gradvel.a23*2);
    tau[p].zz=one_rho2*(twovisc_sps*gradvel.a33 +sumsps);
  }
}
...
//==============================================================================
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity,bool shift>
  void JSphCpu::Interaction_ForcesCpuT(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const
{
...
//==============================================================================
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco,TpDensity tdensity> void JSphCpu::Interaction_Forces_ct5(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const{
  if(MultiPhase){  //<vs_non-Newtonian_ini>
    if(Shifting){ const bool shift=true;
           if(TVelGrad==VELGRAD_FDA)Interaction_ForcesCpuT_NN_FDA<tker,ftmode,tvisco,tdensity,shift>(t,res,Zhan_loop);
      else if(TVelGrad==VELGRAD_SPH)Interaction_ForcesCpuT_NN_SPH<tker,ftmode,tvisco,tdensity,shift>(t,res,Zhan_loop);
    ...
    if(Shifting)Interaction_ForcesCpuT<tker,ftmode,tvisco,tdensity,true >(t,res,Zhan_loop);
    else        Interaction_ForcesCpuT<tker,ftmode,tvisco,tdensity,false>(t,res,Zhan_loop);
}
//==============================================================================
template<TpKernel tker,TpFtMode ftmode,TpVisco tvisco> void JSphCpu::Interaction_Forces_ct4(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const{
       if(TDensity==DDT_None)    Interaction_Forces_ct5<tker,ftmode,tvisco,DDT_None    >(t,res,Zhan_loop);
  else if(TDensity==DDT_DDT)     Interaction_Forces_ct5<tker,ftmode,tvisco,DDT_DDT     >(t,res,Zhan_loop);
  else if(TDensity==DDT_DDT2)    Interaction_Forces_ct5<tker,ftmode,tvisco,DDT_DDT2    >(t,res,Zhan_loop);
  else if(TDensity==DDT_DDT2Full)Interaction_Forces_ct5<tker,ftmode,tvisco,DDT_DDT2Full>(t,res,Zhan_loop);
}
//==============================================================================
template<TpKernel tker,TpFtMode ftmode> void JSphCpu::Interaction_Forces_ct3(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const{
       if(TVisco==VISCO_Artificial)Interaction_Forces_ct4<tker,ftmode,VISCO_Artificial>(t,res,Zhan_loop);
  else if(TVisco==VISCO_LaminarSPS)Interaction_Forces_ct4<tker,ftmode,VISCO_LaminarSPS>(t,res,Zhan_loop);
  else if(TVisco==VISCO_ConstEq   )Interaction_Forces_ct4<tker,ftmode,VISCO_ConstEq   >(t,res,Zhan_loop);
  else if(TVisco==VISCO_Hypoplasticity)Interaction_Forces_ct4<tker,ftmode,VISCO_Hypoplasticity>(t,res,Zhan_loop);
  else if(TVisco==VISCO_Elasticity)Interaction_Forces_ct4<tker,ftmode,VISCO_Elasticity>(t,res,Zhan_loop);
}
//==============================================================================
template<TpKernel tker> void JSphCpu::Interaction_Forces_ct2(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const{
       if(FtMode==FTMODE_None)Interaction_Forces_ct3<tker,FTMODE_None>(t,res,Zhan_loop);
  else if(FtMode==FTMODE_Sph )Interaction_Forces_ct3<tker,FTMODE_Sph >(t,res,Zhan_loop);
  else if(FtMode==FTMODE_Ext )Interaction_Forces_ct3<tker,FTMODE_Ext >(t,res,Zhan_loop);
}
//==============================================================================
void JSphCpu::Interaction_Forces_ct(const stinterparmsc &t,StInterResultc &res,int &Zhan_loop)const{
       if(TKernel==KERNEL_Wendland)  Interaction_Forces_ct2<KERNEL_Wendland>(t,res,Zhan_loop);
  else if(TKernel==KERNEL_Cubic)     Interaction_Forces_ct2<KERNEL_Cubic   >(t,res,Zhan_loop);
}

void JSphCpu::StressRegularization_SPH_ct(unsigned n,unsigned pinit,StDivDataCpu divdata,const unsigned *dcell
  ,tsymatrix3f *tau, const tdouble3 *pos,const tfloat4 *velrhop,const typecode *code,const TpVisco TVisco)const{
       if(TKernel==KERNEL_Wendland) StressRegularization_SPH<KERNEL_Wendland>(n, pinit, divdata, dcell, tau, pos, velrhop, code, TVisco);
  else if(TKernel==KERNEL_Cubic)    StressRegularization_SPH<KERNEL_Cubic   >(n, pinit, divdata, dcell, tau, pos, velrhop, code, TVisco);
 }
...
template<TpKernel tker,bool sim2d,TpSlipMode tslip> void JSphCpu::InteractionMdbcCorrectionT2
  ...
  ,const tfloat3 *boundnormal,bool *BoundCorner,const tfloat3 *motionvel,tfloat4 *velrhop)
{
  ...
  ,const tfloat3 *boundnormal,bool *BoundCorner,const tfloat3 *motionvel,tfloat4 *velrhop)
{
  ...
  if(Simulate2D){ const bool sim2d=true;
    if(slipmode==SLIP_Vel0    )InteractionMdbcCorrectionT2 <tker,sim2d,SLIP_Vel0    > (n,divdata,determlimit,MdbcThreshold,pos,code,idp,boundnormal,BoundCorner,motionvel,velrhop);
    if(slipmode==SLIP_NoSlip  )InteractionMdbcCorrectionT2 <tker,sim2d,SLIP_NoSlip  > (n,divdata,determlimit,MdbcThreshold,pos,code,idp,boundnormal,BoundCorner,motionvel,velrhop);
    if(slipmode==SLIP_FreeSlip)InteractionMdbcCorrectionT2 <tker,sim2d,SLIP_FreeSlip> (n,divdata,determlimit,MdbcThreshold,pos,code,idp,boundnormal,BoundCorner,motionvel,velrhop);
...
{
  switch(TKernel){
    case KERNEL_Cubic:       Interaction_MdbcCorrectionT <KERNEL_Cubic     > (slipmode,divdata,pos,code,idp,boundnormal,BoundCorner,motionvel,velrhop);  break;
    case KERNEL_Wendland:    Interaction_MdbcCorrectionT <KERNEL_Wendland  > (slipmode,divdata,pos,code,idp,boundnormal,BoundCorner,motionvel,velrhop);  break;
  ...
  ,tdouble3 *pos,unsigned *dcell,typecode *code,tfloat4 *velrhopnew)const
{
  ...
  // Log->Printf("\n particle 16916 after running ComputeVerletVarsFluid, pos = %f %f %f ", pos[16916].x, pos[16916].y, pos[16916].z);
  // Log->Printf("New velocity and density = %f %f %f %f ", velrhopnew[16916].x, velrhopnew[16916].y, velrhopnew[16916].z, velrhopnew[16916].w);
  // Log->Printf(" particle 16972 after running ComputeVerletVarsFluid, pos = %f %f %f ", pos[16972].x, pos[16972].y, pos[16972].z);
  // Log->Printf("New velocity and density = %f %f %f %f ", velrhopnew[16972].x, velrhopnew[16972].y, velrhopnew[16972].z, velrhopnew[16972].w);
}
...
void JSphCpu::ComputeVelrhopBound(const tfloat4* velrhopold,double armul,tfloat4* velrhopnew)const{
  ...
  // Log->Printf(" p 6808, After running ComputeVerletrhopBound, velrhopnew = %f %f %f %f", velrhopnew[6808].x, velrhopnew[6808].y, velrhopnew[6808].z, velrhopnew[6808].w);
  // Log->Printf(" p 3059, After running ComputeVerletrhopBound, velrhopnew = %f %f %f %f", velrhopnew[3059].x, velrhopnew[3059].y,velrhopnew[3059].z, velrhopnew[3059].w);
}
...
void JSphCpu::ComputeVerlet(double dt){
  ...
  // printf("Mark: JSphCpu::ComputeVerlet\n");
}
...
void JSphCpu::ComputeSymplecticPre(double dt){
  ...
  for(int p=npb;p<np;p++){
    ...
    if(!WithFloating || CODE_IsFluid(Codec[p])){//-Fluid Particles.
      ...
      // Log->Printf("Point %d Position = %f %f %f", p, PosPrec[p].x, PosPrec[p].y ,PosPrec[p].z);
    }
    else{//-Floating Particles.
  ...
  TmcStop(Timers,TMC_SuComputeStep);
 // Log->Printf("ComputeSymplecticPre");
 // Log->Printf("v11163 at n: %f %f %f",VelrhopPrec[11163].x,VelrhopPrec[11163].y,VelrhopPrec[11163].z);
 // Log->Printf("v11163 at n+1/2,p: %f %f %f",Velrhopc[11163].x,Velrhopc[11163].y,Velrhopc[11163].z);
 // Log->Printf("x11163 at n: %f %f %f",PosPrec[11163].x,PosPrec[11163].y,PosPrec[11163].z);
 // Log->Printf("x11163 at n+1/2,p: %f %f %f",Posc[11163].x,Posc[11163].y,Posc[11163].z);
 // Log->Printf("rho11163 at n: %f",VelrhopPrec[11163].w);
 // Log->Printf("rho11163 at n+1/2,p: %f",Velrhopc[11163].w);
  }
...
void JSphCpu::ComputeSymplecticCorr(double dt){
  ...
  TmcStop(Timers,TMC_SuComputeStep);
 // Log->Printf("ComputeSymplecticCorr");
 // Log->Printf("v11163 at n+1: %f %f %f",Velrhopc[11163].x,Velrhopc[11163].y,Velrhopc[11163].z);
 // Log->Printf("x11163 at n+1: %f %f %f",Posc[11163].x,Posc[11163].y,Posc[11163].z);
 // Log->Printf("rho11163 at n+1: %f",Velrhopc[11163].w);
}

//==============================================================================
/// Update soil particles velocity to n+1 step, prediction value. Do not update position and density for this step
//==============================================================================
void JSphCpu::ComputeSymplecticCorr_ZhanPre(double dt){
   TmcStart(Timers,TMC_SuComputeStep);
  const bool shift=false; //(ShiftingMode!=SHIFT_None); //-We strongly recommend running the shifting correction only for the corrector. If you want to re-enable shifting in the predictor, change the value here to "true".
  // PosPrec, VelrhopPrec: n step, save, unchange
  //Posc, Velrhopc: n+1/2 step, need to update to n + 1 predictor step
  const double dt05=dt*.5;
  ...
  for(int p=npb;p<np;p++){
    if(!WithFloating || CODE_IsFluid(Codec[p])){//-Fluid Particles.
      //-Calculate displacement. | Calcula desplazamiento.
      double dx=double(Velrhopc[p].x)*dt05;
      double dy=double(Velrhopc[p].y)*dt05;
      double dz=double(Velrhopc[p].z)*dt05;
      if(shift){
        dx+=double(ShiftPosfsc[p].x);
        dy+=double(ShiftPosfsc[p].y);
        dz+=double(ShiftPosfsc[p].z);
      ...
      Velrhopc[p].x += (Acec[p].x + Gravity.x) * dt05; 
      Velrhopc[p].y += (Acec[p].y + Gravity.y) * dt05; 
      Velrhopc[p].z += (Acec[p].z + Gravity.z) * dt05; 
      //-Restore data of inout particles.
      //if(InOut && CODE_IsFluidInout(Codec[p])){
      //  outrhop=false;
      //  rvelrhopnew=Velrhopc[p];
      //  const tfloat3 vd=indirvel[CODE_GetIzoneFluidInout(Codec[p])];
      //  if(vd.x!=FLT_MAX){
      //    const float v=rvelrhopnew.x*vd.x + rvelrhopnew.y*vd.y + rvelrhopnew.z*vd.z;
      //    dx=double(v*vd.x) * dt05;
      ...
      //}
      //-Update particle data.
      Posc[p].x += dx; Posc[p].y += dy; Posc[p].z += dz;
      //UpdatePos(Posc[p],dx,dy,dz,outrhop,p,Posc,Dcellc,Codec);
      // Log->Printf("Point %d Position = %f %f %f", p, PosPrec[p].x, PosPrec[p].y ,PosPrec[p].z);
    }
    else{//-Floating Particles. // Not checked
      Velrhopc[p]=VelrhopPrec[p];
      //Velrhopc[p].w=(rhopnew<RhopZero? RhopZero: rhopnew); //-Avoid fluid particles being absorbed by floating ones. | Evita q las floating absorvan a las fluidas.
      //-Copy position. | Copia posicion.
      Posc[p]=PosPrec[p];
  ...
  TmcStop(Timers,TMC_SuComputeStep);
 // Log->Printf("step11* v n: %f %f %f",VelrhopPrec[62].x,VelrhopPrec[62].y,VelrhopPrec[62].z);
  //Log->Printf("step11* v n+1,p: %f %f %f",Velrhopc[62].x,Velrhopc[62].y,Velrhopc[62].z);
 // Log->Printf("step11* x n: %f %f %f",PosPrec[62].x,PosPrec[62].y,PosPrec[62].z);
 // Log->Printf("step11* x n+1,p: %f %f %f",Posc[62].x,Posc[62].y,Posc[62].z);
 // Log->Printf("step11* rho n: %f",VelrhopPrec[62].w);
 // Log->Printf("step11* rho n+1,p: %f",Velrhopc[62].w);
}
//==============================================================================
// Update soil particles position and density to n+1 step,  corrected value. No update for velocity at this step
//==============================================================================
void JSphCpu::ComputeSymplecticCorr_ZhanCorr(double dt){
  TmcStart(Timers,TMC_SuComputeStep);
  const bool shift=(Shifting!=NULL);
  const int npb=int(Npb);
  #ifdef OMP_USE
  ...
  for(int p=npb;p<np;p++){
    ...
    if(!WithFloating || CODE_IsFluid(Codec[p])){//-Fluid Particles.
      //-Calculate velocity & density. | Calcula velocidad y densidad.
      tfloat4 rvelrhopnew=TFloat4(
        float(double(Velrhopc[p].x)), 
        float(double(Velrhopc[p].y)), 
        float(double(Velrhopc[p].z)),
        rhopnew);
      //-Calculate displacement. | Calcula desplazamiento.
      double dx=(double(VelrhopPrec[p].x)+double(rvelrhopnew.x)) * dt05; 
      double dy=(double(VelrhopPrec[p].y)+double(rvelrhopnew.y)) * dt05; 
  ...
  ArraysCpu->Free(VelrhopPrec);  VelrhopPrec=NULL;
  TmcStop(Timers,TMC_SuComputeStep);
 // Log->Printf("v n+1: %f %f %f",Velrhopc[62].x,Velrhopc[62].y,Velrhopc[62].z);
 // Log->Printf("x n+1: %f %f %f",Posc[62].x,Posc[62].y,Posc[62].z);
 // Log->Printf("rho n+1: %f",Velrhopc[62].w);
}
...
double JSphCpu::DtVariable(bool final){
  ...
  // printf("Mark: JSphCpu DtVariable\n");
  return(dt);
}
...
void JSphCpu::MoveMatBound(unsigned np,unsigned ini,tmatrix4d m,double dt,const unsigned *ridpmv
  ,tdouble3 *pos,unsigned *dcell,tfloat4 *velrhop,typecode *code,tfloat3 *boundnormal,bool *boundcorner)const
{
  const unsigned fin=ini+np;
  for(unsigned id=ini;id<fin;id++){
    const unsigned pid=RidpMove[id];
...
void JSphCpu::RunMotion(double stepdt){
  ...
  bool *boundcorner=NULL;
  boundnormal=BoundNormalc;
  boundcorner=BoundCornerc;
  ...
  if(DsMotion->GetActiveMotion()){
    ...
    for(unsigned ref=0;ref<nref;ref++){
      ...
      if(m.type==MOTT_Matrix){//-Matrix movement (for rotations).
        if(motsim)MoveMatBound   (m.count,m.idbegin-CaseNfixed,m.matmov,stepdt,RidpMove,Posc,Dcellc,Velrhopc,Codec,boundnormal,boundcorner); 
