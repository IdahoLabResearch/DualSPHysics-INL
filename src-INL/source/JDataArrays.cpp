...
void JDataArrays::FreeMemory(StDataArray &arr){
  if(arr.delptr){
    ...
    switch(arr.type){
      ...
      case TypeBool:     delete[] (bool    *)ptr;  break;
...
void JDataArrays::ReverseArrayData(unsigned idx){
  ...
  switch(ar.type){
    ...
    case TypeBool:{    bool   *v=(bool  *)ar.ptr;  for(unsigned c=0;c<count ;c++)v[c]=-v[c];  }break;
...
//==============================================================================
/// Returns dynamic pointer with bool array. (this pointer must be deleted)
//==============================================================================
bool* JDataArrays::NewArrayBool(unsigned count,bool defvalue,bool value){
  try{
    bool *v=new bool[count];
    if(count && defvalue){
      if(!value)memset(v,0,sizeof(bool)*count);
      else for(unsigned c=0;c<count;c++)v[c]=value;
    }
    return(v);
  }
...
unsigned JDataArrays::FilterApply(unsigned count,const byte *filter){
  ...
  for(unsigned c=0;c<na;c++){
    ...
    switch(arr.type){
      ...
      case TypeBool:     ReindexData(nsel,reindex,arr.count,(bool    *)arr.ptr,NULL);  break;
      default: Run_Exceptioon(fun::PrintStr("Type of pointer \'%s\' is invalid.",TypeToStr(arr.type)));
    }
    arr.count=nsel;
  }
...
unsigned JDataArrays::SortData(unsigned count,const unsigned *reindex){
  ...
  for(unsigned c=0;c<na;c++){
    ...
    switch(arr.type){
      ...
      case TypeBool:     ReindexData(count,reindex,arr.count,(bool    *)arr.ptr,(bool    *)aux);  break;
      default: Run_Exceptioon(fun::PrintStr("Type of pointer \'%s\' is invalid.",TypeToStr(arr.type)));
    }
    arr.count=count;
  }
