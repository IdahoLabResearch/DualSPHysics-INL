...
void JSphCpuSingle::ConfigDomain()
{
  ...
  if (FtMotSave)
    ConfigFtMotionSave(Np, Posc, Idpc); //<vs_ftmottionsv>
  ...
  // printf("mark tvisco=%d", TVisco);
  if (MultiPhase)
    LoadMultiphaseData(Np, Idpc, Codec, Velrhopc, AuxNN); //<vs_non-Newtonian>

  //-Load normals for boundary particles (fixed and moving).
  if (UseNormals)
    LoadBoundNormals(Np, Npb, Idpc, Codec, BoundNormalc, BoundCornerc);
  ...
  RunInitialize(Np, Npb, Posc, Idpc, Codec, Velrhopc, boundnormal, BoundCorner);
  if (UseNormals)
    ConfigBoundNormals(Np, Npb, Posc, Idpc, BoundNormalc, BoundCornerc);
  ...
  CellDivSingle = new JCellDivCpuSingle(Stable, FtCount != 0, PeriActive, CellMode, Scell, Map_PosMin, Map_PosMax, Map_Cells, CaseNbound, CaseNfixed, CaseNpb, DirOut);
...
/// Este kernel vale para single-cpu y multi-cpu porque usa domposmin.
//==============================================================================
void JSphCpuSingle::PeriodicDuplicateVerlet(unsigned np, unsigned pini, tuint3 cellmax, tdouble3 perinc, const unsigned *listp, unsigned *idp, typecode *code, unsigned *dcell, tdouble3 *pos, tfloat4 *velrhop, float *auxnn, tfloat4 *velrhopm1) const
{
  const int n = int(np);
#ifdef OMP_USE
#pragma omp parallel for schedule(static) if (n > OMP_LIMIT_COMPUTELIGHT)
  ...
  for (int p = 0; p < n; p++)
  {
    ...
    velrhopm1[pnew] = velrhopm1[pcopy];
    if (auxnn)
      auxnn[pnew] = auxnn[pcopy]; //<vs_non-Newtonian>
  }
}
...
/// Este kernel vale para single-cpu y multi-cpu porque usa domposmin.
//==============================================================================
void JSphCpuSingle::PeriodicDuplicateSymplectic(unsigned np, unsigned pini, tuint3 cellmax, tdouble3 perinc, const unsigned *listp, unsigned *idp, typecode *code, unsigned *dcell, tdouble3 *pos, tfloat4 *velrhop, tsymatrix3f *spstau, tdouble3 *pospre, tfloat4 *velrhoppre) const
{
  const int n = int(np);
#ifdef OMP_USE
#pragma omp parallel for schedule(static) if (n > OMP_LIMIT_COMPUTELIGHT)
...
/// Este kernel vale para single-cpu y multi-cpu porque usa domposmin.
//==============================================================================
void JSphCpuSingle::PeriodicDuplicateSymplectic(unsigned np, unsigned pini, tuint3 cellmax, tdouble3 perinc, const unsigned *listp, unsigned *idp, typecode *code, unsigned *dcell, tdouble3 *pos, tfloat4 *velrhop, float *auxnn, tdouble3 *pospre, tfloat4 *velrhoppre) const
{
  const int n = int(np);
#ifdef OMP_USE
#pragma omp parallel for schedule(static) if (n > OMP_LIMIT_COMPUTELIGHT)
...
/// Este kernel vale para single-cpu y multi-cpu porque usa domposmin.
//==============================================================================
void JSphCpuSingle::PeriodicDuplicateNormals(unsigned np, unsigned pini, tuint3 cellmax, tdouble3 perinc, const unsigned *listp, tfloat3 *normals, bool *boundcorner, tfloat3 *motionvel) const
{
  const int n = int(np);
#ifdef OMP_USE
#pragma omp parallel for schedule(static) if (n > OMP_LIMIT_COMPUTELIGHT)
...
void JSphCpuSingle::RunPeriodic()
{
  ...
  for (unsigned p = 0; p < Np; p++)
  {
    ...
    if (CODE_IsPeriodic(rcode))
      Codec[p] = CODE_SetOutIgnore(rcode);
  }
  ...
  NpbPer = NpfPer = 0;
  BoundChanged = true;
  for (unsigned ctype = 0; ctype < 2; ctype++)
  { //-0:bound, 1:fluid+floating.
    ...
    for (unsigned cper = 0; cper < 3; cper++)
      if ((cper == 0 && PeriX) || (cper == 1 && PeriY) || (cper == 2 && PeriZ))
      {
        ...
        for (unsigned cblock = 0; cblock < 2; cblock++)
        {                                                  //-0:new periodic, 1:original particles. | 0:periodicas nuevas, 1:particulas originales
          ...
          while (run && num2)
          {
            ...
            if (Np >= 0x80000000)
              Run_Exceptioon("The number of particles is too big."); //-Because the last bit is used to mark the direction in which a new periodic particle is created. | Porque el ultimo bit se usa para marcar el sentido en que se crea la nueva periodica.
            ...
            else
            {
              ...
              {
                if (!MultiPhase)
                  PeriodicDuplicateVerlet(count, Np, DomCells, perinc, listp, Idpc, Codec, Dcellc, Posc, Velrhopc, SpsTauc, VelrhopM1c); //<vs_non-Newtonian>
                else
                  PeriodicDuplicateVerlet(count, Np, DomCells, perinc, listp, Idpc, Codec, Dcellc, Posc, Velrhopc, AuxNN, VelrhopM1c); //<vs_non-Newtonian>
              }
              if (TStep == STEP_Symplectic)
              {
                ...
                if (!MultiPhase)
                  PeriodicDuplicateSymplectic(count, Np, DomCells, perinc, listp, Idpc, Codec, Dcellc, Posc, Velrhopc, SpsTauc, PosPrec, VelrhopPrec); //<vs_non-Newtonian>
                else
                  PeriodicDuplicateSymplectic(count, Np, DomCells, perinc, listp, Idpc, Codec, Dcellc, Posc, Velrhopc, AuxNN, PosPrec, VelrhopPrec); //<vs_non-Newtonian>
              }
              if (UseNormals)
                PeriodicDuplicateNormals(count, Np, DomCells, perinc, listp, BoundNormalc, BoundCornerc, MotionVelc);
...
void JSphCpuSingle::RunCellDivide(bool updateperiodic)
{
  ...
  { //-In reality, this is only necessary in divide for corrector, not in predictor??? | En realidad solo es necesario en el divide del corrector, no en el predictor???
    if (!PosPrec || !VelrhopPrec)
      Run_Exceptioon("Symplectic data is invalid.");
    CellDivSingle->SortArray(PosPrec);
    CellDivSingle->SortArray(VelrhopPrec);
  }
  // printf("mark--runcellDevide function-0\n");
  //  sort stress for hypoplastic model
  if ((!MultiPhase && TVisco == VISCO_LaminarSPS) || (TVisco == VISCO_Hypoplasticity) || (TVisco == VISCO_Elasticity))
  {
    CellDivSingle->SortArray(SpsTauc); //<vs_non-Newtonian>
    CellDivSingle->SortArray(Forcec);
    if (TVisco == VISCO_Hypoplasticity)
      CellDivSingle->SortArray(Void_ratioc);
  }
  // printf("mark--runcellDevide function-1\n");
  ...
  if (UseNormals)
  {
    ...
    if (MotionVelc)
      CellDivSingle->SortArray(MotionVelc);
  }
  ...
  if (CellDivSingle->GetNpbOut())
    AbortBoundOut();

  //-Collect position of floating particles. | Recupera posiciones de floatings.
  if (CaseNfloat)
    CalcRidp(PeriActive != 0, Np - Npb, Npb, CaseNpb, CaseNpb + CaseNfloat, Codec, Idpc, FtRidp);
  ...
  if (npfout)
  {
    ...
    // printf("mark--runcellDevide function\n");
...
//==============================================================================
void JSphCpuSingle::Interaction_Forces(TpInterStep interstep, int Zhan_loop)
{
  if (TBoundary == BC_MDBC && (MdbcCorrector || interstep != INTERSTEP_SymCorrector))
    MdbcBoundCorrection(); //-Boundary correction for mDBC.
  InterStep = interstep;
  PreInteraction_Forces(interstep, Zhan_loop);
  ...
  const stinterparmsc parms = StInterparmsc(Np, Npb, NpbOk, DivData, Dcellc, Posc, Velrhopc, Idpc, Codec, Pressc, Arc, Acec, Deltac, ShiftingMode, ShiftPosfsc, SpsTauc,Forcec, SpsGradvelc, Visco_etac, Void_ratioc, D_tensorc, W_tensorc, AuxNN, BoundNormalc, BoundCornerc //<vs_non-Newtonian>
  );
  StInterResultc res;
  res.viscdt = 0;
  res.viscetadt = 0; //<vs_non-Newtonian>
  // printf("mark JSphCpuSingle Interaction_Forces - 1");
  JSphCpu::Interaction_Forces_ct(parms, res, Zhan_loop);
...
void JSphCpuSingle::MdbcBoundCorrection()
{
  ...
  Interaction_MdbcCorrection(SlipMode, DivData, Posc, Codec, Idpc, BoundNormalc, BoundCornerc, MotionVelc, Velrhopc);
...
double JSphCpuSingle::ComputeAceMax(unsigned np, const tfloat3 *ace, const typecode *code) const
{
  ...
  if (check)
    return (ComputeAceMaxOmp<true>(Np - Npb, Acec + Npb, Codec + Npb));
  else
    return (ComputeAceMaxOmp<false>(Np - Npb, Acec + Npb, Codec + Npb));
}
...
double JSphCpuSingle::ComputeAceMaxSeq(unsigned np, const tfloat3 *ace, const typecode *code) const
{
  float acemax = 0;
  const int n = int(np);
  for (int p = 0; p < n; p++)
...
//==============================================================================
template <bool checkcode>
double JSphCpuSingle::ComputeAceMaxOmp(unsigned np, const tfloat3 *ace, const typecode *code) const
{
  double acemax = 0;
#ifdef OMP_USE
  if (np > OMP_LIMIT_COMPUTELIGHT)
  {
    const int n = int(np);
    if (n < 0)
      Run_Exceptioon("Number of values is too big.");
    float amax = 0;
#pragma omp parallel
    {
      float amax2 = 0;
...
#pragma omp critical
    ...
    acemax = ComputeAceMaxSeq<checkcode>(np, ace, code);
#else
  if (np)
    acemax = ComputeAceMaxSeq<checkcode>(np, ace, code);
#endif
...
double JSphCpuSingle::ComputeStep_Ver()
{
  int Zhan_loop = 0;
  if (BoundCorr)
    BoundCorrectionData();                         //-Apply BoundCorrection.
  Interaction_Forces(INTERSTEP_Verlet, Zhan_loop); //-Interaction.
  const double dt = DtVariable(true);              //-Calculate new dt.
  if (CaseNmoving)
    CalcMotion(dt); //-Calculate motion for moving bodies.
  DemDtForce = dt;  //(DEM)
  if (Shifting)
    RunShifting(dt); //-Shifting.
  ComputeVerlet(dt); //-Update particles using Verlet.
  if (CaseNfloat)
    RunFloating(dt, false);     //-Control of floating bodies.
  PosInteraction_Forces(false); //-Free memory used for interaction.
  if (Damping)
    RunDamping(dt, Np, Npb, Posc, Codec, Velrhopc); //-Applies Damping.
  if (RelaxZones)
    RunRelaxZone(dt); //-Generate waves using RZ.
  return (dt);
}
...
double JSphCpuSingle::ComputeStep_Sym()
{
  ...
  //-Predictor
  //-----------
  int Zhan_loop = 0; // Indicator for Zhan_friction slip mode: 0: not used
  bool Zhan_method = false;
  if (SlipMode == SLIP_Friction_Zhan || SlipMode == SLIP_NoSlip_Zhan || SlipMode == SLIP_FreeSlip_Zhan)
  {
    Zhan_method = true;
  }
  DemDtForce = dt * 0.5f; //(DEM)
  Time_inc = dt * 0.5f;   // time increment for stress integration
  if (BoundCorr)
    BoundCorrectionData(); //-Apply BoundCorrection.
  if (!Zhan_method)
    Interaction_Forces(INTERSTEP_SymPredictor, Zhan_loop); //-Interaction.
  else
  {
    Zhan_loop = 1; // Indicator for Zhan_friction slip mode: 1: predictor step, ignore boundary
    Interaction_Forces(INTERSTEP_SymPredictor, Zhan_loop);
  }
  const double ddt_p = DtVariable(false); //-Calculate dt of predictor step.
  if (Shifting)
    RunShifting(dt * .5);   //-Shifting.
  ComputeSymplecticPre(dt); //-Apply Symplectic-Predictor to particles (periodic particles become invalid). No matter use Zhan_friction or not.
  if (SlipMode == SLIP_Friction_Zhan || SlipMode == SLIP_NoSlip_Zhan || SlipMode == SLIP_FreeSlip_Zhan)
  {
    Zhan_loop = 2;                // Indicator for Zhan_friction slip mode: 2: Only goes to fluid-bound interaction, determine rate of penetraiton and add correction force
    PosInteraction_Forces(false); //-Free memory used for interaction.
    Interaction_Forces(INTERSTEP_SymPredictor, Zhan_loop);
  }
  if (CaseNfloat)
    RunFloating(dt * .5, true); //-Control of floating bodies.
  PosInteraction_Forces(false); //-Free memory used for interaction.
  //-Corrector
  //-----------
  DemDtForce = dt; //(DEM)
  RunCellDivide(true);
  if (!Zhan_method)
    Interaction_Forces(INTERSTEP_SymCorrector, 0); //-Interaction.
  else
  {
    Zhan_loop = 1; // Indicator for Zhan_friction slip mode: 1: predictor step, ignore boundary
    Interaction_Forces(INTERSTEP_SymCorrector, Zhan_loop);
  }
  ...
  {
    ComputeSymplecticCorr_ZhanPre(dt); // Predictor velocity and position for n+1 step
    Zhan_loop = 2;
    PosInteraction_Forces(true); //-Free memory used for interaction.
    Interaction_Forces(INTERSTEP_SymCorrector, Zhan_loop);
    ComputeSymplecticCorr_ZhanCorr(dt);
  }
  if (CaseNfloat)
    RunFloating(dt, false);     //-Control of floating bodies.
  PosInteraction_Forces(false); //-Free memory used for interaction.
...
#pragma omp parallel for schedule(guided)
  ...
  for (int cf = 0; cf < ftcount; cf++)
  {
    ...
    if (FtLinearForce != NULL || FtAngularForce != NULL)
      FtSumExternalForces(cf, face, fomegaace);
  ...
  for (unsigned cf = 0; cf < FtCount; cf++)
    if (!FtObjs[cf].usechrono && (FtLinearVel[cf] != NULL || FtAngularVel[cf] != NULL))
    {
      if (FtLinearVel[cf] != NULL)
      {
        ...
        if (v.x != FLT_MAX)
          FtoForcesRes[cf].fvelres.x = v.x;
        if (v.y != FLT_MAX)
          FtoForcesRes[cf].fvelres.y = v.y;
        if (v.z != FLT_MAX)
          FtoForcesRes[cf].fvelres.z = v.z;
      ...
      if (FtAngularVel[cf] != NULL)
      {
        ...
        if (v.x != FLT_MAX)
          FtoForcesRes[cf].fomegares.x = v.x;
        if (v.y != FLT_MAX)
          FtoForcesRes[cf].fomegares.y = v.y;
        if (v.z != FLT_MAX)
          FtoForcesRes[cf].fomegares.z = v.z;
      }
    }
}
...
{
  if (TimeStep >= FtPause)
  { //-Operator >= is used because when FtPause=0 in symplectic-predictor, code would not enter here. | Se usa >= pq si FtPause es cero en symplectic-predictor no entraria.
    ...
    //-Applies imposed velocity.
    if (FtLinearVel != NULL)
      FtApplyImposedVel(FtoForcesRes);
    //-Applies motion constraints.
    if (FtConstraints)
      FtApplyConstraints(FtoForces, FtoForcesRes);
...
#pragma omp parallel for schedule(guided)
    ...
    if (!predictor && ForcePoints)
    {
      ...
      if (Moorings)
        Moorings->ComputeForces(Nstep, TimeStep, dt, ForcePoints);
...
void JSphCpuSingle::ComputePips(bool run)
{
  if (run || DsPips->CheckRun(Nstep))
  {
    ...
    DsPips->ComputeCpu(Nstep, TimeStep, timesim, CSP, OmpThreads, Np, Npb, NpbOk, DivData, Dcellc, Posc);
  }
}
...
void JSphCpuSingle::Run(std::string appname, JSphCfgRun *cfg, JLog2 *log)
{
  ...
  //--------------------------------------------------------------------------------
  LoadConfig(cfg); /// has loadCaceConfig function to load xml input parameters, the same as the cpu version
  LoadCaseParticles();
  VisuConfig();
  ConfigDomain(); /// reserve memory for spsTauc
  ConfigRunMode(cfg);
  VisuParticleSummary();

  //-Initialisation of execution variables. | Inicializacion de variables de ejecucion.
  //------------------------------------------------------------------------------------
  InitRunCpu(); /// initialize stress and void ratio in spsTauc
  RunGaugeSystem(TimeStep, true);
  if (InOut)
    InOutInit(TimeStepIni);
  FreePartsInit();
  UpdateMaxValues();
  PrintAllocMemory(GetAllocMemoryCpu());
  SaveData(); /// save variable
  ...
  while (TimeStep < TimeMax)
  {
    ...
    if (ViscoTime)
      Visco = ViscoTime->GetVisco(float(TimeStep));
    double stepdt = ComputeStep();
    RunGaugeSystem(TimeStep + stepdt);
    if (CaseNmoving)
      RunMotion(stepdt);
    if (InOut)
      InOutComputeStep(stepdt);
    else
      RunCellDivide(true);
    TimeStep += stepdt;
    LastDt = stepdt;
    ...
    if (DsPips)
      ComputePips(laststep);
    if (Part <= PartIni + 1 && tc.CheckTime())
      Log->Print(string("  ") + tc.GetInfoFinish((TimeStep - TimeStepIni) / (TimeMax - TimeStepIni)));
    if (NstepsBreak && Nstep >= NstepsBreak)
      break; //-For debugging.
  }
  TimerSim.Stop();
  TimerTot.Stop();
...
void JSphCpuSingle::SaveData()
{
  ...
  typecode *code = NULL;
  tfloat3 *Sigma = NULL; // hypoplastic model stress diagonal components
  tfloat3 *Force = NULL; // boundary reaction force components
  float *aux_void = NULL;
  if (save)
  {
    //-Assign memory and collect particle values. | Asigna memoria y recupera datos de las particulas.
    // printf("mark--SaveData\n");
    idp = ArraysCpu->ReserveUint();
    pos = ArraysCpu->ReserveDouble3();
    vel = ArraysCpu->ReserveFloat3();
    rhop = ArraysCpu->ReserveFloat();
    // printf("Todo: hypoplasticity stress invariants output error\n");
    if (TVisco == VISCO_Hypoplasticity)
    {
      Sigma = ArraysCpu->ReserveFloat3();
      aux_void = ArraysCpu->ReserveFloat();
      Force = ArraysCpu->ReserveFloat3();
    }
    ...
    //  printf("mark--SaveData -1 \n");
    if (MultiPhase && TVisco != VISCO_Hypoplasticity && TVisco != VISCO_Elasticity)
      aux_n = ArraysCpu->ReserveFloat(); //<vs_non-Newtonian>
    // if (MultiPhase)code = ArraysCpu->ReserveTypeCode();	//<vs_non-Newtonian>
    //  printf("mark--SaveData -2 \n");
    unsigned npnormal = GetParticlesData(Np, 0, PeriActive != 0, idp, pos, vel, rhop, code, aux_n, Sigma, aux_void, Force);
  ...
  // printf("mark--SaveData -4 \n");
  if (Sigma)
    arrays.AddArray("Sigma", npsave, Sigma);
  if (aux_void)
    arrays.AddArray("Void", npsave, aux_void);
  if (Force)
    arrays.AddArray("Force", npsave, Force);
  // printf("mark--SaveData -5 \n");
  JSph::SaveData(npsave, arrays, 1, vdom, &infoplus);
  // printf("mark--SaveData -6 \n");
  ...
  ArraysCpu->Free(Sigma); // hypoplasticity
  ArraysCpu->Free(aux_void);
  ArraysCpu->Free(Force);
  if (UseNormals && SvNormals)
    SaveVtkNormals("normals/Normals.vtk", Part, npsave, Npb, Posc, Idpc, BoundNormalc);
